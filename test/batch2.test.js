...
function fakeDb(mode = 'sales') {
  ...
  const db = {
    ...
    async query(sql) {
      if (sql.startsWith('SELECT id,status FROM quotations'))
        return [[{ id: 'q1', status: state.status }]];
      if (sql.startsWith('UPDATE quotations')) {
        state.status = 'sent';
        return [[]];
      }
      if (sql.startsWith('SELECT id,status FROM work_orders'))
        return [[{ id: 'w1', status: state.status }]];
      if (sql.startsWith('UPDATE work_orders')) {
      ...
      if (
        sql.startsWith(
          'SELECT id,status,warehouse_id,so_id,customer_id FROM delivery_challans',
        )
      )
        return [[{ id: 'd1', status: 'draft', warehouse_id: 'w1' }]];
      if (sql.startsWith('SELECT id FROM dispatch_effects'))
        return [state.effect ? [{ id: 'effect' }] : []];
      if (
        sql.startsWith(
          'SELECT id,item_id,quantity,order_item_id FROM delivery_challan_items',
        )
      )
        return [[{ id: 'di1', item_id: 'i1', quantity: 2 }]];
      if (sql.startsWith('SELECT current_qty,avg_rate'))
        return [[{ current_qty: state.qty, avg_rate: 5 }]];
      if (sql.startsWith('UPDATE stock_summary')) {
        state.qty -= 2;
        return [[]];
      }
      ...
      if (sql.startsWith('INSERT INTO dispatch_effects')) {
        state.effect = true;
        return [[]];
      }
...
test('invalid production/sales transitions are rejected', async () => {
  ...
  await assert.rejects(
    () => transition(db, 'quotations', 'q1', 'paid', 'user-1'),
    /Cannot transition/,
  );
});
...
test('job card rejects invalid state transitions', async () => {
  const db = {
    ...
    async query(sql) {
      if (sql.startsWith('SELECT id,status FROM job_cards'))
        return [[{ id: 'jc1', status: 'queued' }]];
      throw new Error(`Unexpected query: ${sql}`);
    },
  };
  await assert.rejects(
    () => transitionJobCard(db, 'jc1', 'completed', 'user-1'),
    /Cannot transition job card/,
  );
});
...
test('production output normalizes decimal strings and rejects invalid or excessive quantities', () => {
  ...
  assert.throws(
    () => resolveOutputQuantity('0.000', '10.000', '0.000'),
    /positive/,
  );
  assert.throws(
    () => resolveOutputQuantity('0.000', '10.000', '11.000'),
    /cannot exceed/,
  );
  assert.throws(
    () => resolveOutputQuantity('not-a-number', '10.000', 'not-a-number'),
    /positive/,
  );
});
...
test('production output requires a matching released production-order relationship', () => {
  ...
  assert.throws(
    () => assertProductionLink(null, order, 10),
    /matching released production order/,
  );
  assert.throws(
    () =>
      assertProductionLink(
        { status: 'draft', bom_id: 'bom-1', item_id: 'fg-1', planned_qty: 10 },
        order,
        10,
      ),
    /matching released production order/,
