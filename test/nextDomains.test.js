...
test('employee creation validates required identity fields', async () => {
  ...
  assert.throws(() => service.assertInput(service.DEFINITIONS.employees, req.body), /name is required/);
});
...
test('idempotent domain creation returns the original response', async () => {
  ...
  const db = {
    async query(sql, options) {
      calls.push(sql);
      if (sql.startsWith('SELECT response_json')) return [[{ response_json: JSON.stringify({ id: 'existing' }) }]];
  ...
  const result = await service.create(req, 'employees', { employee_code: 'E-1', name: 'A' });
  assert.deepEqual(result, { id: 'existing' });
  assert.equal(calls.length, 1);
});

test('finance journals fail closed when debits and credits do not balance', async () => {
  const req = { body: { journal_number: 'J-1', journal_date: '2026-01-01', lines: [
    { account_id: 'cash', debit: 100 }, { account_id: 'sales', credit: 99 }
  ] }, orgDb: {}, user: { sub: 'u1' } };
  await assert.rejects(() => service.createJournal(req, req.body), /must balance/);
});
...
test('domain writes commit the audit and idempotency record together', async () => {
  ...
  const req = {
    body: { employee_code: 'E-2', name: 'B' }, orgDb: {
    ...
    }, get: () => 'create-e2', user: { sub: 'u1' }, ip: '127.0.0.1'
  };
  ...
  assert.ok(calls.some(sql => sql.startsWith('INSERT INTO domain_idempotency')));
