...
test('partial dispatch cannot authorize full invoicing including repeated item lines', () => {
  ...
  assert.throws(
    () => assertFullyDispatched(order, [{ item_id: 'a', quantity: 6 }]),
    /All order quantities/,
  );
  assert.doesNotThrow(() =>
    assertFullyDispatched(order, [{ item_id: 'a', quantity: 10 }]),
  );
  assert.throws(
    () => assertFullyDispatched(order, [{ item_id: 'b', quantity: 10 }]),
    /All order quantities/,
  );
});
test('conflicting legacy and canonical order identities fail closed', () => {
  assert.throws(
    () => salesOrderId({ so_id: 'a', order_id: 'b' }),
    /Conflicting/,
  );
...
test('invoice money rejects nonfinite inputs and preserves zero GST', () => {
  assert.throws(
    () => priceLines([{ item_id: 'a', quantity: NaN, rate: 1 }], false),
    /Invalid/,
  );
  const [line] = priceLines(
    [{ item_id: 'a', quantity: 2, rate: 100, gst_rate: 0 }],
    false,
  );
  ...
  const db = {
    ...
    query: async (sql) => {
      ...
      if (sql.includes('FROM sales_orders'))
        return [[{ id: 'so', customer_id: 'c', status: 'delivered' }]];
      if (sql.includes('FROM invoices'))
        return [[{ id: 'inv', customer_id: 'c', so_id: 'so' }]];
...
test('wrong customer cannot replay an order invoice', async () => {
  ...
  const db = {
    ...
    query: async () => [
      [{ id: 'so', customer_id: 'owner', status: 'delivered' }],
    ],
  };
  await assert.rejects(
    createInvoice(db, { so_id: 'so', customer_id: 'other' }, 'u'),
