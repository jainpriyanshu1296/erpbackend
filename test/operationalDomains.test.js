...
test('payroll snapshot prorates payable days and preserves deduction totals', () => {
  const result = service.payrollSnapshot(
    { salary: 120000 },
    { annual_ctc: 120000, components: { basic: 10000, allowances: 2000 } },
    { working_days: 20, present_days: 10 },
    { pf: 500, tax: 100 },
  );
  assert.deepEqual(result, {
    ...
    deductions: 600,
    net: 5400,
    components: { basic: 10000, allowances: 2000 },
    attendance: { working_days: 20, present_days: 10 },
  });
});

test('GST calculation returns immutable authoritative totals', () => {
  const result = service.calculateGSTAuthoritative(
...
test('government documents are durable adapter boundaries, not unsupported claims', () => {
  assert.equal(
    service.integrationBoundary('einvoice', 'inv-1', {}).status,
    'pending',
  );
  assert.throws(
    () => service.integrationBoundary('gstn', 'x', {}),
    /Unsupported/,
  );
});
...
test('reports reject arbitrary SQL and validate pagination ranges', () => {
  ...
  assert.throws(
    () => service.reportDefinition('DROP TABLE invoices'),
    /not available/,
  );
  assert.deepEqual(service.range({ from: '2026-01-01', to: '2026-01-31' }), [
    '2026-01-01',
    '2026-01-31',
  ]);
  assert.throws(
    () => service.range({ from: '2026-02-01', to: '2026-01-01' }),
    /Invalid date range/,
  );
});
...
test('finance account creation rejects indirect hierarchy cycles', async () => {
  const db = {
    async query(sql) {
      if (String(sql).includes('WHERE id=? AND is_active=1'))
        return [[{ id: 'parent', parent_id: 'ancestor' }]];
...
test('journal posting fails when no accounting period covers the journal date', async () => {
  const db = {
    ...
    async query(sql) {
      ...
      if (text.includes('SELECT * FROM finance_journals'))
        return [
          [{ id: 'j-1', status: 'approved', journal_date: '2026-09-19' }],
        ];
