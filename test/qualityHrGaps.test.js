...
function buildReq({ stockCurrent = 200 } = {}) {
  const db = {
    async query(sql) {
      ...
      if (text.includes('select coalesce(max(snapshot_version)'))
        return [{ next_version: 0 }];
      if (text.includes('select * from quality_ncrs'))
        return [[{ id: 'ncr-1', status: 'open' }]];
      if (text.includes('select * from qc_inspections'))
        return [
          [
            {
              id: 'insp-1',
      ...
      if (text.includes('select current_qty,avg_rate from stock_summary'))
        return [[{ current_qty: stockCurrent, avg_rate: 5 }]];
      if (text.includes('select * from attendance_corrections'))
        return [
          [
            {
              id: 'corr-1',
      ...
      if (text.includes('select holiday_date from leave_holidays'))
        return [[{ holiday_date: '2026-09-19' }]];
      if (text.includes('select * from leave_balances'))
        return [
          [{ employee_id: 'emp-1', leave_type: 'annual', closing_balance: 10 }],
        ];
      return [[]];
    },
    async transaction() {
...
test('quality specification + inspection snapshot creation are durable and auditable', async () => {
  ...
  const snapshot = await service.createInspectionSnapshot(req, 'insp-1', {
    result: 'pass',
    accepted_qty: 18,
  });
...
test('disposition stock effects reduce summary and close inspection', async () => {
  ...
  assert.equal(result.stock_effect.balance_qty, 48);
});
...
test('leave validation and attendance correction approval protect human-resource controls', async () => {
  ...
  const correction = await service.approveAttendanceCorrection(req, 'corr-1', {
    status: 'approved',
  });
...
test('NCR lifecycle rejects skipped states and terminal mutation', async () => {
  ...
  const req = {
    orgDb: {
      ...
      async query(sql) {
        if (sql.includes('SELECT * FROM quality_ncrs'))
          return [[{ id: 'ncr-1', status: 'open' }]];
        return [[]];
      },
    },
  ...
  await assert.rejects(
    () => service.transitionNcr(req, 'ncr-1', { status: 'closed' }),
    /cannot transition/,
  );
});
...
test('financial statements only aggregate posted journals and use account normal sign', async () => {
  ...
  const result = await service.financialStatement(req, 'income_statement', {
    from: '2026-01-01',
    to: '2026-01-31',
  });
