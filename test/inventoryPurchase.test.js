...
test('GRN posting awaits Incoming QC without crediting stock and is idempotent', async () => {
  ...
  const db = {
    query: async (sql, options = {}) => {
      if (sql.startsWith('SELECT * FROM grn_items'))
        return [[{ item_id: 'item-1', quantity: 3, rate: 10 }]];
      if (sql.startsWith('SELECT * FROM grn'))
        return [[{ id: 'grn-1', status, warehouse_id: 'wh-1' }]];
      if (
      ...
      if (sql.startsWith('INSERT INTO stock_ledger')) {
        ledger.push(options.replacements);
        return [];
      }
      if (sql.startsWith('INSERT INTO stock_summary')) {
        summary.push(options.replacements);
        return [];
      }
      if (sql.startsWith('UPDATE grn')) {
        status = 'posted';
        return [];
      }
      return [];
...
test('warehouse transfer receipt is idempotent', async () => {
  ...
  assert.equal(
    (await service.receiveTransfer(db, 't-1', 'u-1')).status,
    'received',
  );
  assert.equal(
    (await service.receiveTransfer(db, 't-1', 'u-1')).alreadyReceived,
    true,
  );
...
test('reservation refuses to exceed available stock', async () => {
  const db = {
    query: async (sql) => {
      ...
      if (sql.startsWith('SELECT quantity FROM stock_reservations'))
        return [[{ quantity: 4 }]];
      return [];
    },
    transaction: async (fn) => fn({}),
  ...
  const result = await service.reserveStock(
    db,
    { item_id: 'i-1', warehouse_id: 'w-1', quantity: 2 },
    'u-1',
  );
...
test('RFQ transition uses the explicit comparison and selection workflow', async () => {
  const db = {
    query: async (sql) =>
      sql.startsWith('SELECT * FROM rfqs')
        ? [[{ id: 'r-1', status: 'quoted' }]]
        : [],
    transaction: async (fn) => fn({}),
  };
  assert.equal(
    (await service.transitionRfq(db, 'r-1', 'compared', 'u-1')).status,
    'compared',
  );
});
