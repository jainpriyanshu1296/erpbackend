...
async function runStockCheck() {
  try {
    const [orgs] = await masterDb.query(
      'SELECT db_name, company_name FROM organizations WHERE is_active=1 AND is_suspended=0',
    );
    for (const org of orgs) {
      try {
        ...
        await sendLowStockAlertToOwner(orgDb).catch((err) =>
          console.warn(`[WHATSAPP LOW STOCK] ${org.db_name}:`, err.message),
        );
      } catch (orgErr) {
