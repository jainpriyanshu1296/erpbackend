...
router.get('/categories', permission('inventory','can_view'), asyncHandler(async (req,res) => {
  const {page,limit,offset,search,sort,direction} = listQuery(req.query,['category','is_active']);
  const filters=search ? ['category LIKE ?'] : [], values = search ? [`%${search}%`] : [];
  ...
  const [[count]] = await req.orgDb.query(`SELECT COUNT(*) total FROM ${categorySource}${where}`,{replacements:values});
  const [rows] = await req.orgDb.query(`SELECT category id,category,is_active FROM ${categorySource}${where} ORDER BY ${sort} ${direction} LIMIT ? OFFSET ?`,{replacements:[...values,limit,offset]});
  return ok(res,rows,'Item groups fetched',{page,limit,total:Number(count.total)});
}));
for (const method of ['post','put']) router[method](method === 'post' ? '/categories' : '/categories/:id', permission('inventory',method === 'post' ? 'can_create' : 'can_edit'), asyncHandler(async (req,res) => {
  const category = typeof req.body.category === 'string' ? req.body.category.trim() : '';
  if (!/^[a-zA-Z0-9][a-zA-Z0-9 _-]{0,29}$/.test(category) || (req.body.is_active !== undefined && req.body.is_active !== '' && ![0,1,'0','1'].includes(req.body.is_active))) return fail(res,400,'VALIDATION_ERROR','Category must contain 1–30 letters, numbers, spaces, underscores or hyphens');
  const active = req.body.is_active === '' || req.body.is_active === undefined ? 1 : Number(req.body.is_active);
  const tx = await req.orgDb.transaction();
  try {
    const [[old]] = await req.orgDb.query(`SELECT category FROM ${categorySource} WHERE category=?`,{replacements:[req.params.id || category],transaction:tx});
    if (method === 'post' && old) throw Object.assign(new Error('Category already exists'),{status:409,code:'CONFLICT'});
    if (method === 'put' && !old) throw Object.assign(new Error('Category not found'),{status:404,code:'NOT_FOUND'});
    if (method === 'put' && req.params.id !== category) {
      const [[target]] = await req.orgDb.query(`SELECT category FROM ${categorySource} WHERE category=?`,{replacements:[category],transaction:tx});
      if (target) throw Object.assign(new Error('Category already exists'),{status:409,code:'CONFLICT'});
      await req.orgDb.query('UPDATE item_master SET category=? WHERE category=?',{replacements:[category,req.params.id],transaction:tx});
      await req.orgDb.query('DELETE FROM company_settings WHERE setting_key=?',{replacements:[`inventory.category.${req.params.id}`],transaction:tx});
    }
    await req.orgDb.query('INSERT INTO company_settings(setting_key,setting_value) VALUES(?,?) ON DUPLICATE KEY UPDATE setting_value=VALUES(setting_value)',{replacements:[`inventory.category.${category}`,String(active)],transaction:tx});
    await tx.commit(); return ok(res,{id:category,category,is_active:active});
  } catch (cause) { await tx.rollback(); throw cause; }
}));

router.get('/uom', permission('inventory', 'can_view'), asyncHandler(async (req, res) => {
  const { page, limit, offset, search, sort, direction } = listQuery(req.query, ['uom_code','uom_name','id']);
  const filters = [], values = [];
  if (search) { filters.push('(uom_code LIKE ? OR uom_name LIKE ?)'); values.push(`%${search}%`, `%${search}%`); }
  if (req.query.is_active !== undefined) {
    if (!['0','1'].includes(req.query.is_active)) return fail(res,400,'VALIDATION_ERROR','Invalid active filter');
    filters.push('is_active=?'); values.push(Number(req.query.is_active));
  }
  const where = filters.length ? ` WHERE ${filters.join(' AND ')}` : '';
  const [[count]] = await req.orgDb.query(`SELECT COUNT(*) total FROM uom_master${where}`, { replacements: values });
  const [rows] = await req.orgDb.query(`SELECT * FROM uom_master${where} ORDER BY ${sort} ${direction},id LIMIT ? OFFSET ?`, { replacements: [...values,limit,offset] });
  return ok(res,rows,'Units fetched',{page,limit,total:Number(count.total)});
}));
router.get('/uom/:id', permission('inventory','can_view'), asyncHandler(async (req,res) => {
  const [[row]] = await req.orgDb.query('SELECT * FROM uom_master WHERE id=?',{replacements:[req.params.id]});
  return row ? ok(res,row) : fail(res,404,'NOT_FOUND','Unit not found');
}));
for (const method of ['post','put']) router[method](method === 'post' ? '/uom' : '/uom/:id', permission('inventory', method === 'post' ? 'can_create' : 'can_edit'), asyncHandler(async (req,res) => {
  const { uom_code: code, uom_name: name } = req.body;
  if (typeof code !== 'string' || !code.trim() || code.length>20 || typeof name !== 'string' || !name.trim() || name.length>100 || (req.body.is_active !== undefined && req.body.is_active !== '' && ![0,1,'0','1'].includes(req.body.is_active))) return fail(res,400,'VALIDATION_ERROR','Unit code, name and valid active state are required');
  const active = req.body.is_active === undefined || req.body.is_active === '' ? 1 : Number(req.body.is_active);
  if (method === 'put') {
    const [[existing]] = await req.orgDb.query('SELECT id FROM uom_master WHERE id=?',{replacements:[req.params.id]});
    if (!existing) return fail(res,404,'NOT_FOUND','Unit not found');
    await req.orgDb.query('UPDATE uom_master SET uom_code=?,uom_name=?,is_active=? WHERE id=?',{replacements:[code.trim(),name.trim(),active,req.params.id]});
    return ok(res,{id:req.params.id,uom_code:code.trim(),uom_name:name.trim(),is_active:active});
  }
  const [result] = await req.orgDb.query('INSERT INTO uom_master(uom_code,uom_name,is_active) VALUES(?,?,?)',{replacements:[code.trim(),name.trim(),active]});
  return created(res,{id:result,uom_code:code.trim(),uom_name:name.trim(),is_active:active});
}));

router.get('/reports', permission('inventory','can_view'), asyncHandler(async (req,res) => {
  const { page,limit,offset,search,sort,direction } = listQuery(req.query,['item_code','item_name','current_qty','total_value']);
  const where = search ? ' WHERE im.item_code LIKE ? OR im.item_name LIKE ?' : '';
  const values = search ? [`%${search}%`,`%${search}%`] : [];
  const [[count]] = await req.orgDb.query(`SELECT COUNT(*) total FROM item_master im${where}`,{replacements:values});
  const [rows] = await req.orgDb.query(`SELECT im.id,im.item_code,im.item_name,im.reorder_level,COALESCE(SUM(ss.current_qty),0) current_qty,COALESCE(SUM(ss.total_value),0) total_value FROM item_master im LEFT JOIN stock_summary ss ON ss.item_id=im.id${where} GROUP BY im.id ORDER BY ${sort} ${direction},im.id LIMIT ? OFFSET ?`,{replacements:[...values,limit,offset]});
  return ok(res,rows,'Stock valuation',{page,limit,total:Number(count.total)});
}));
...
router.get('/transfers/:id',permission('inventory','can_view'),asyncHandler(async(req,res)=>{
  const [[transfer]]=await req.orgDb.query('SELECT * FROM warehouse_transfers WHERE id=?',{replacements:[req.params.id]});
  if(!transfer) return fail(res,404,'NOT_FOUND','Transfer not found');
  const [items]=await req.orgDb.query('SELECT wti.*,im.item_code,im.item_name FROM warehouse_transfer_items wti JOIN item_master im ON im.id=wti.item_id WHERE wti.transfer_id=? ORDER BY wti.id',{replacements:[req.params.id]});
...
router.post('/transfers/:id/post', permission('inventory', 'can_edit'), asyncHandler(async (req, res) => {
  const result = await require('./inventoryPurchase.service').receiveTransfer(req.orgDb, req.params.id, req.user.sub);
  if (result.error) return fail(res, result.error === 'NOT_FOUND' ? 404 : 409, result.error, 'Unable to post transfer');
  return ok(res, result, result.alreadyReceived ? 'Transfer was already posted' : 'Transfer posted');
}));
...
router.post('/adjustments', permission('inventory', 'can_create'), asyncHandler(async (req, res) => {
  ...
  for (const item of items) {
    if (!item.item_id || !Number.isFinite(Number(item.quantity)) || Number(item.quantity)<=0 || !Number.isFinite(Number(item.rate || 0)) || Number(item.rate || 0)<0 || !['increase', 'decrease','in','out'].includes(item.direction)) {
  ...
  try {
    ...
    for (const item of items) {
      await req.orgDb.query(`
        ...
        VALUES(?, ?, ?, ?, ?, ?)
      `, {
        replacements: [uuid(), adjustmentId, item.item_id, ['increase','in'].includes(item.direction) ? 'in' : 'out', item.quantity, item.rate || 0],
        transaction: tx
      });
    }
...
router.post('/adjustments/:id/post', permission('inventory', 'can_edit'), asyncHandler(async (req, res) => {
  return ok(res, await require('../services/erp.service').postStockAdjustment(req.orgDb, { id: req.params.id }, req.user.sub), 'Adjustment posted');
}));
