...
const {
  ...
  issueMaterials,
  transitionJobCard,
} = require('../services/salesProduction.service');
...
sales.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const [[count]] = await req.orgDb.query(
      ...
      { replacements: search ? replacements.slice(0, 3) : [] },
    );
    const [rows] = await req.orgDb.query(
      `SELECT id,customer_code,company_name,contact_person,phone,email,gstin,state,is_active,created_at FROM customers${where} ORDER BY created_at DESC LIMIT ? OFFSET ?`,
      { replacements },
    );
    return ok(res, rows, 'Customers fetched', {
      limit,
      offset,
...
sales.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const [[customer]] = await req.orgDb.query(
      'SELECT * FROM customers WHERE id=? LIMIT 1',
      { replacements: [id] },
    );
    ...
    (SELECT COALESCE(SUM(balance_amount),0) FROM invoices WHERE customer_id=?) outstanding_amount`,
      { replacements: [id, id, id, id, id] },
    );
    const [timeline] = await req.orgDb.query(
      `SELECT created_at,event_type,reference_id,description FROM (
  ...
  ) events ORDER BY created_at DESC LIMIT 100`,
...
for (const [path, table, columns] of [
  ['orders', 'production_orders', ['production_number', 'item_id', 'status']],
  ['bom', 'bom', ['bom_code', 'finished_item_id', 'version_no']],
  ['work-orders', 'work_orders', ['wo_number', 'finished_item_id', 'status']],
  ['job-cards', 'job_cards', ['production_order_id', 'status']],
]) {
  production.get(
    `/${path}`,
    ...
    asyncHandler(async (req, res) => {
      ...
      if (req.query.status && columns.includes('status')) {
        clauses.push('status=?');
        values.push(req.query.status);
      }
      if (req.query.status && table === 'bom') {
        clauses.push('is_active=?');
        values.push(req.query.status === 'active' ? 1 : 0);
      }
      const where = clauses.length ? ` WHERE ${clauses.join(' AND ')}` : '';
      const [[count]] = await req.orgDb.query(
        `SELECT COUNT(*) total FROM ${table}${where}`,
        { replacements: values },
      );
      const [rows] = await req.orgDb.query(
        `SELECT *${table === 'bom' ? ",CASE WHEN is_active=1 THEN 'active' ELSE 'archived' END status" : ''} FROM ${table}${where} ORDER BY ${sort} ${direction},id LIMIT ? OFFSET ?`,
...
production.post(
  ...
  asyncHandler(async (req, res) =>
    ok(
      res,
      await require('../services/salesProduction.service').releaseProductionOrder(
        req.orgDb,
        req.params.id,
        req.user.sub,
      ),
...
production.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const [rows] = await req.orgDb.query(
      `SELECT * FROM work_orders${where} ORDER BY ${sort} ${direction},id LIMIT ? OFFSET ?`,
      { replacements: [...values, limit, offset] },
    );
    return ok(res, rows, 'Production progress', {
      page,
      limit,
      total: Number(count.total),
    });
  ...
  asyncHandler(async (req, res) => {
    ...
    await req.orgDb.query(
      'INSERT INTO production_orders(id,production_number,so_id,bom_id,item_id,planned_qty,status,created_by) VALUES(?,?,?,?,?,?,?,?)',
      {
        replacements: [
          id,
          req.body.production_number || null,
          salesOrderId || null,
          bomId,
          itemId,
          Number(plannedQty),
...
production.put(
  ...
  asyncHandler(async (req, res) => {
    ...
    return ok(res, {
      id: req.params.id,
      is_active: req.body.status === 'active' ? 1 : 0,
    });
  }),
);
production.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    const [[bom]] = await req.orgDb.query(
      'SELECT id FROM bom WHERE id=? AND finished_item_id=? AND is_active=1',
      { replacements: [bom_id || null, finished_item_id || null] },
    );
    if (
      !bom ||
...
production.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    try {
      ...
      if (existing) {
        await tx.commit();
        return ok(res, { id: existing.id, already_created: true });
      }
      const id = uuid();
      await req.orgDb.query(
        'INSERT INTO bom(id,bom_code,finished_item_id,output_qty,version_no,is_active,created_by) VALUES(?,?,?,?,?,0,?)',
        {
          replacements: [
            id,
            req.body.bom_code,
            ...
            bom.output_qty,
            Number(bom.version_no || 1) + 1,
            req.user.sub,
          ],
          transaction: tx,
        },
      );
      const [components] = await req.orgDb.query(
      ...
      return created(res, {
        id,
        bom_code: req.body.bom_code,
        version_no: Number(bom.version_no || 1) + 1,
      });
    } catch (cause) {
      await tx.rollback();
      throw cause;
...
production.get(
  ...
  asyncHandler(async (req, res) => {
    const [[orders]] = await req.orgDb.query(
      'SELECT COUNT(*) total FROM work_orders',
    );
    const [[open]] = await req.orgDb.query(
      "SELECT COUNT(*) total FROM work_orders WHERE status IN ('released','in_progress')",
    );
    const [[cards]] = await req.orgDb.query(
      "SELECT COUNT(*) total FROM job_cards WHERE status NOT IN ('completed','cancelled')",
    );
    return ok(res, {
      work_orders: Number(orders.total || 0),
      open_work_orders: Number(open.total || 0),
      open_job_cards: Number(cards.total || 0),
    });
  }),
);
