...
const resources = {
  approvals: [
    'approval',
    ['entity_type', 'entity_id', 'requested_by', 'reason'],
    'settings',
  ],
  'rfq/suppliers': [
    'rfqSupplier',
    ['rfq_id', 'supplier_id', 'status'],
    'purchase',
  ],
  'rfq/quotation-lines': [
    'quotationLine',
  ...
  batches: [
    'batch',
    ['item_id', 'batch_no', 'expiry_date', 'quantity', 'warehouse_id'],
    'inventory',
  ],
  serials: [
    'serial',
    ['item_id', 'serial_no', 'status', 'warehouse_id'],
    'inventory',
  ],
  'physical-counts': [
    'count',
    ['count_number', 'warehouse_id', 'status'],
    'inventory',
  ],
  'physical-counts/lines': [
    'countLine',
  ...
  'warehouse-locations': [
    'location',
    ['warehouse_id', 'code', 'name', 'parent_id', 'is_active'],
    'inventory',
  ],
  imports: [
    'importJob',
  ...
  exports: [
    'exportJob',
    ['entity_type', 'filter_json', 'status', 'result_json'],
    'reports',
  ],
  'sales-enquiries': [
    'enquiry',
    ['enquiry_number', 'customer_id', 'status', 'expected_date', 'notes'],
    'sales',
  ],
  'payment-allocations': [
    'allocation',
  ...
  'production-outputs': [
    'output',
    ['production_order_id', 'item_id', 'quantity', 'batch_id', 'warehouse_id'],
    'production',
  ],
  'production-downtime': [
    'downtime',
  ...
  'production-scrap': [
    'scrap',
    ['production_order_id', 'item_id', 'quantity', 'reason', 'warehouse_id'],
    'production',
  ],
  'related-documents': [
    'related',
...
const resourceByPath = Object.fromEntries(
  Object.entries(resources).map(([path, [table, , module]]) => [
    path,
    { table, module },
  ]),
);
  ...
  router.get(
    ...
    asyncHandler(async (req, res) => {
      ...
      if (search) {
        ...
        values.push(...searchKeys.map(() => `%${search}%`));
      }
      if (req.query.status && keys.includes('status')) {
        clauses.push('status=?');
        values.push(req.query.status);
      }
      const where = clauses.length ? ` WHERE ${clauses.join(' AND ')}` : '';
      const [[count]] = await req.orgDb.query(
        `SELECT COUNT(*) total FROM ${service.TABLES[table]}${where}`,
        { replacements: values },
      );
      const [rows] = await req.orgDb.query(
        `SELECT * FROM ${service.TABLES[table]}${where} ORDER BY ${sort} ${direction},id LIMIT ? OFFSET ?`,
  ...
  asyncHandler(async (req, res, next) => {
    ...
    if (
      ...
      return fail(
        ...
        'Status transitions are not supported for this resource',
      );
    }
...
router.post(
  '/closure/credit-notes/:id/issue',
  moduleGuard('sales'),
  permission('sales', 'can_edit'),
  asyncHandler(async (req, res) => {
    return ok(
      res,
      await service.issueCreditNote(req.orgDb, req.params.id, req.user?.sub),
      'Credit note issued',
    );
  }),
);
router.post(
  '/closure/payment-allocations',
  moduleGuard('sales'),
  permission('sales', 'can_edit'),
  asyncHandler(async (req, res) => {
    return created(
...
router.post(
  '/closure/physical-counts/:id/post',
  moduleGuard('inventory'),
  permission('inventory', 'can_edit'),
  asyncHandler(async (req, res) => {
    return ok(
      res,
      await service.postPhysicalCount(req.orgDb, req.params.id, req.user?.sub),
      'Physical count posted',
    );
  }),
);
router.post(
  '/closure/production-outputs/:id/post',
  moduleGuard('production'),
  permission('production', 'can_edit'),
  asyncHandler(async (req, res) => {
    return ok(
      res,
      await service.postProductionEffect(
        req.orgDb,
        'output',
        req.params.id,
        req.user?.sub,
...
router.post(
  '/closure/production-scrap/:id/post',
  moduleGuard('production'),
  permission('production', 'can_edit'),
  asyncHandler(async (req, res) => {
    return ok(
...
router.post(
  '/closure/imports/:id/process',
  moduleGuard('inventory'),
  permission('inventory', 'can_edit'),
  asyncHandler(async (req, res) => {
    return ok(
      res,
      await service.processImport(req.orgDb, req.params.id, req.user?.sub),
      'Import processed',
    );
  }),
);
router.post(
  '/closure/exports/:id/run',
  moduleGuard('reports'),
  permission('reports', 'can_view'),
  asyncHandler(async (req, res) => {
    return ok(
      res,
      await service.processExport(req.orgDb, req.params.id, req.user?.sub),
      'Export generated',
    );
  }),
);
...
router.get(
  '/closure/:resource/:id/related-documents',
      ...
      await permission(definition.module, 'can_edit')(req, res, async () => {
        const status =
          req.params.action === 'approve'
            ? definition.table === 'creditNote'
              ? 'issued'
              : definition.table === 'approval'
                ? 'approved'
                : 'posted'
            : 'cancelled';
        return ok(
...
router.post(
  '/closure/rfq/quotation-lines/:id/select',
  moduleGuard('purchase'),
  permission('purchase', 'can_edit'),
  asyncHandler(async (req, res) => {
    return ok(
      res,
      await service.selectQuotation(req.orgDb, req.params.id, req.user?.sub),
      'Quotation selected',
    );
  }),
);
  ...
  asyncHandler(async (req, res) => {
    ...
    try {
      ...
      await req.orgDb.query(
        'INSERT INTO approval_steps(id,approval_id,step_no,approver_role) VALUES(?,?,?,?)',
        {
          replacements: [id, req.params.id, step_no, approver_role],
          transaction: tx,
        },
      );
      await service.audit(
