    ...
    for (const item of items) {
      ...
      if (
        ...
        return fail(
          ...
          400,
          'VALIDATION_ERROR',
          'Each item needs valid quantity, rate, discount and GST values',
        );
      }
    }
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    return result.already_converted
      ? ok(res, result, 'Sales order already exists')
      : created(res, result, 'Quotation converted to sales order');
  }),
);
    ...
    try {
      ...
      if (so_id) {
        ...
        for (const [itemId, returnQty] of requested) {
          const [[sent]] = await req.orgDb.query(
            "SELECT COALESCE(SUM(dci.quantity),0) quantity FROM delivery_challan_items dci JOIN delivery_challans dc ON dc.id=dci.challan_id WHERE dc.so_id=? AND dci.item_id=? AND dc.status IN ('dispatched','delivered') FOR UPDATE",
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    try {
      const [ret] = await req.orgDb.query(
        'SELECT status, warehouse_id FROM sales_returns WHERE id = ? FOR UPDATE',
      ...
      for (const item of items) {
        ...
        await require('../services/zeroGapClosure.service').applyStockEffect(
          req.orgDb,
          {
            operationKey: `sales-return:${req.params.id}:${item.id}`,
      ...
      await req.orgDb.query(
        'UPDATE sales_returns SET status = ?, posted_at = NOW() WHERE id = ?',
        {
      ...
      return ok(
        ...
        'Sales return posted and inventory updated',
      );
    } catch (error) {
      await tx.rollback();
      throw error;
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const [rows] = await req.orgDb.query(
      `SELECT i.*,c.company_name FROM invoices i LEFT JOIN customers c ON c.id=i.customer_id${where} ORDER BY i.created_at DESC LIMIT ? OFFSET ?`,
      { replacements: [...values, limit, (page - 1) * limit] },
    );
    return ok(res, rows, 'Invoices fetched', {
      page,
      limit,
      total: Number(count.total || 0),
    });
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    return created(
      res,
      await require('../services/invoice.service').createInvoice(
        req.orgDb,
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    return ok(
      res,
      await require('../services/salesProduction.service').transition(
        req.orgDb,
        'invoices',
        req.params.id,
        'issued',
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const [payments] = await req.orgDb.query(
  ...
  asyncHandler(async (req, res) => {
    ...
    return created(
      ...
      await require('../services/erp.service').recordInvoicePayment(
        ...
        req.params.id,
        req.body.amount,
        { ...req.body, idempotency_key: `sales-payment:${key}` },
        req.user.sub,
      ),
    );
  }),
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const where = search
      ? ' WHERE p.reference LIKE ? OR i.invoice_number LIKE ? OR c.company_name LIKE ?'
      : '';
    const values = search ? [`%${search}%`, `%${search}%`, `%${search}%`] : [];
    const from =
      ' FROM invoice_payments p JOIN invoices i ON i.id=p.invoice_id LEFT JOIN customers c ON c.id=i.customer_id';
    const [[count]] = await req.orgDb.query(
      `SELECT COUNT(*) total${from}${where}`,
      { replacements: values },
    );
    const [rows] = await req.orgDb.query(
      `SELECT p.*,i.invoice_number,c.company_name${from}${where} ORDER BY p.created_at DESC,p.id LIMIT ? OFFSET ?`,
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    const result =
      await require('../services/salesProduction.service').dispatch(
        req.orgDb,
        req.params.id,
    ...
    result.invoice =
      await require('../services/automation.service').handleDeliveryChallanSaved(
        req.orgDb,
        req.params.id,
        req.user.sub,
      );
