...
async function issueRefresh(orgDb, user, org) {
  ...
  await orgDb.query(
    ...
    { replacements: [uuid(), user.id, '', hash] },
  );
  return token;
}
const login = asyncHandler(async (req, res) => {
  ...
  return ok(
    res,
    {
      ...
      org: { id: org.id, slug: org.slug, company_name: org.company_name },
    },
    'Login successful',
  );
});
...
const refresh = asyncHandler(async (req, res) => {
  ...
  if (!rows.length) {
    ...
    return fail(
      res,
      401,
      reused.length ? 'REFRESH_TOKEN_REUSE' : 'UNAUTHENTICATED',
  ...
  await orgDb.query(
    ...
    { replacements: [replacementId, rows[0].id] },
  );
  const token = signToken(users[0], orgs[0]);
  const refresh_token = await issueRefresh(orgDb, users[0], orgs[0]);
  setSessionCookies(res, token, refresh_token);
...
const forgotPassword = asyncHandler(async (req, res) => {
  ...
  if (users.length) {
    ...
    await sendEmail({
      ...
      text: `Reset your password: ${base}/reset-password?token=${raw}`,
    });
  }
  return ok(res, null, 'If the account exists, reset instructions were sent');
});
