...
function crud(table, module = 'dashboard', options = {}) {
  ...
  r.get(
    ...
    asyncHandler(async (req, res) => {
      ...
      const [rows] = await req.orgDb.query(
        `SELECT * FROM ${table}${where} ORDER BY 1 DESC LIMIT ? OFFSET ?`,
        { replacements },
      );
  ...
  r.delete(
    ...
    asyncHandler(async (req, res) => {
      await req.orgDb.query(`DELETE FROM ${table} WHERE id = ?`, {
        replacements: [req.params.id],
      });
      ...
      r[method || 'put'](
        ...
        asyncHandler(async (req, res) => {
          await req.orgDb.query(`UPDATE ${table} SET status = ? WHERE id = ?`, {
            replacements: [path.replace(/-/g, '_'), req.params.id],
          });
          return ok(
            res,
            { id: req.params.id, status: path.replace(/-/g, '_') },
            'Status updated',
          );
        }),
      );
