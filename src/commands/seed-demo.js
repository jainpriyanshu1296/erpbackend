...
async function main() {
  if (process.argv.includes('--help')) {
    console.log(
      'Usage: npm run seed:demo -- --org-password <6 digits> --admin-password <6 digits>',
    );
    return;
  }
  ...
  const [existing] = await masterDb.query(
    'SELECT id, db_name FROM organizations WHERE slug=? OR db_name=? LIMIT 1',
    {
  ...
  try {
    ...
    await masterDb.query(
      ...
      {
        replacements: [uuid(), organizationId, hostname, DEMO_SLUG],
        transaction: tx,
      },
    );
    await masterDb.query(
    ...
    if (subscriptions.length) {
      ...
      await masterDb.query(
        ...
        {
          replacements: [subscriptionId, organizationId, DEMO_PLAN, total],
          transaction: tx,
        },
      );
    }
      ...
      await masterDb.query(
         ...
         ON DUPLICATE KEY UPDATE amount=VALUES(amount)`,
        {
          replacements: [
            ...
            subscriptionId,
            module.module_key,
            Number(modulePrices[0]?.amount || 0),
          ],
          transaction: tx,
        },
      );
  ...
  try {
    ...
    if (users.length) {
      await root.query(
        ...
        [tenantHash, 'Demo Owner', users[0].id],
      );
    } else {
      await root.query(
        'INSERT INTO users(id,name,email,password_hash,role,is_active) VALUES(?,?,?,?,?,1)',
        [uuid(), 'Demo Owner', DEMO_EMAIL, tenantHash, 'admin'],
      );
    }
    await root.end();
