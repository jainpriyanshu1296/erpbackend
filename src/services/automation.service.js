...
async function handleSalesOrderConfirmed(orgDb, salesOrderId, userId) {
  ...
  const [orders] = await orgDb.query(
  ...
  for (const item of soItems) {
    ...
    if (rules.auto_pr_on_shortfall) {
      ...
      for (const comp of bomComponents) {
        ...
        const requiredQty =
          Number(comp.quantity) * Number(item.quantity) * scrapFactor;
        ...
        if (availableQty < requiredQty) {
          const deficit =
            Math.round((requiredQty - availableQty) * 1000) / 1000;
          shortfallItems.push({
  ...
  if (shortfallItems.length > 0 && rules.auto_pr_on_shortfall) {
    ...
    createdPr = {
      id: prId,
      pr_number: prNumber,
      items_count: shortfallItems.length,
    };
...
async function handleWorkOrderCompleted(
  ...
  const [[warehouse]] = await orgDb.query(
    'SELECT id FROM warehouses WHERE is_active=1 ORDER BY is_default DESC,id LIMIT 1',
  );
...
async function handleQcInspectionResult(orgDb, qcInspectionId, userId) {
  return require('./qualityInspection.service').processResult(
    orgDb,
    qcInspectionId,
    {},
    userId,
