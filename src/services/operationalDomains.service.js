...
const REPORTS = {
  receivables: {
    sql: 'SELECT i.id,i.invoice_number,i.invoice_date,i.total_amount,i.balance_amount,i.status FROM invoices i WHERE i.balance_amount > 0 AND i.invoice_date BETWEEN ? AND ? ORDER BY i.invoice_date DESC',
    module: 'finance',
  },
  payables: {
    sql: 'SELECT id,document_number,document_date,amount,status,due_date FROM finance_documents WHERE document_type="payable" AND document_date BETWEEN ? AND ? ORDER BY document_date DESC',
    module: 'finance',
  },
  ar_ageing: {
    sql: 'SELECT i.id,i.invoice_number,i.invoice_date,i.due_date,i.balance_amount,CASE WHEN DATEDIFF(CURDATE(),COALESCE(i.due_date,i.invoice_date))<=0 THEN "current" WHEN DATEDIFF(CURDATE(),COALESCE(i.due_date,i.invoice_date))<=30 THEN "1-30" WHEN DATEDIFF(CURDATE(),COALESCE(i.due_date,i.invoice_date))<=60 THEN "31-60" WHEN DATEDIFF(CURDATE(),COALESCE(i.due_date,i.invoice_date))<=90 THEN "61-90" ELSE "90+" END ageing_bucket FROM invoices i WHERE i.balance_amount>0 AND i.invoice_date BETWEEN ? AND ? ORDER BY i.due_date',
    module: 'finance',
  },
  ap_ageing: {
    sql: 'SELECT f.id,f.document_number,f.document_date,f.due_date,f.amount,CASE WHEN DATEDIFF(CURDATE(),COALESCE(f.due_date,f.document_date))<=0 THEN "current" WHEN DATEDIFF(CURDATE(),COALESCE(f.due_date,f.document_date))<=30 THEN "1-30" WHEN DATEDIFF(CURDATE(),COALESCE(f.due_date,f.document_date))<=60 THEN "31-60" WHEN DATEDIFF(CURDATE(),COALESCE(f.due_date,f.document_date))<=90 THEN "61-90" ELSE "90+" END ageing_bucket FROM finance_documents f WHERE f.document_type="payable" AND f.status NOT IN ("paid","cancelled") AND f.document_date BETWEEN ? AND ? ORDER BY f.due_date',
    module: 'finance',
  },
  attendance: {
    sql: 'SELECT employee_id,COUNT(*) total_days,SUM(status="absent") absent_days FROM attendance WHERE attendance_date BETWEEN ? AND ? GROUP BY employee_id ORDER BY absent_days DESC',
    module: 'hr',
  },
  quality: {
    sql: 'SELECT status,COUNT(*) inspections FROM qc_inspections WHERE created_at BETWEEN ? AND ? GROUP BY status',
    module: 'quality',
  },
  gst: {
    sql: 'SELECT filing_period,SUM(taxable_amount) taxable_amount,SUM(cgst) cgst,SUM(sgst) sgst,SUM(igst) igst FROM tax_transactions WHERE transaction_date BETWEEN ? AND ? GROUP BY filing_period ORDER BY filing_period',
    module: 'gst',
  },
  payroll: {
    sql: 'SELECT pr.run_number,pr.period_start,pr.period_end,pr.status,COUNT(pp.id) payslips,COALESCE(SUM(pp.gross_amount),0) gross_amount,COALESCE(SUM(pp.net_amount),0) net_amount FROM payroll_runs pr LEFT JOIN payroll_payslips pp ON pp.payroll_run_id=pr.id WHERE pr.period_start >= ? AND pr.period_end <= ? GROUP BY pr.id ORDER BY pr.period_end DESC',
    module: 'payroll',
  },
  leave: {
    sql: 'SELECT status,COUNT(*) requests,COALESCE(SUM(days),0) days FROM leave_requests WHERE from_date >= ? AND to_date <= ? GROUP BY status ORDER BY status',
    module: 'hr',
  },
  ncr: {
    sql: 'SELECT status,severity,COUNT(*) count FROM quality_ncrs WHERE created_at BETWEEN ? AND ? GROUP BY status,severity ORDER BY status,severity',
    module: 'quality',
  },
};
...
function payrollSnapshot(
  ...
  const monthly =
    Number(components.basic ?? annual / 12) +
    Number(components.allowances || 0);
  const days = Math.max(1, Number(attendance.working_days || 26));
  const payable = Math.min(
    days,
    Math.max(
      0,
      Number(attendance.present_days ?? days) +
        Number(attendance.paid_leave || 0),
    ),
  );
  ...
  return {
    ...
    deductions: Math.round(total * 100) / 100,
    net: Math.round((gross - total) * 100) / 100,
    components,
    attendance,
  };
}
function calculateGSTAuthoritative(items, orgState, customerState) {
  ...
  return {
    ...
    totals,
    interstate: Boolean(
      orgState && customerState && orgState !== customerState,
    ),
    source: 'internal-gst-calculator-v1',
  };
}
function integrationBoundary(type, sourceId, payload) {
  ...
  return {
    ...
    integration: 'provider-adapter-required',
  };
}
...
function range(params = {}) {
  ...
  const start =
    params.from || `${new Date(`${end}T00:00:00Z`).getUTCFullYear()}-01-01`;
  if (
...
async function createNcr(req, body) {
  ...
  try {
    const [[inspection]] = await req.orgDb.query(
      'SELECT id,rejected_qty,status FROM qc_inspections WHERE id=? FOR UPDATE',
    ...
    await req.orgDb.query(
      'INSERT INTO quality_ncrs(id,ncr_number,inspection_id,severity,description,status,owner_id) VALUES(?,?,?,?,?,?,?)',
      {
        replacements: [
          id,
          body.ncr_number,
    ...
    await req.orgDb.query(
      'INSERT INTO quality_ncr_events(id,ncr_id,event_type,actor_id,event_note) VALUES(?,?,?,?,?)',
      {
        replacements: [
          ...
          'created',
          req.user?.sub || null,
          'NCR created from rejected inspection',
        ],
        transaction: tx,
      },
    );
...
async function finalizePayroll(req, runId) {
  const tx = await req.orgDb.transaction();
  try {
    const [run] = await req.orgDb.query(
    ...
    if (
      ...
      const [snapshots] = await req.orgDb.query(
        'SELECT COUNT(*) AS count FROM payroll_snapshots WHERE payroll_run_id=?',
        { replacements: [runId], transaction: tx },
      );
      await tx.commit();
      return {
        id: runId,
        status: run[0].status,
        snapshot_count: Number(snapshots[0].count),
    ...
    for (const item of items) {
      const payload =
        typeof item.payload === 'string'
          ...
          : item.payload || item;
      await req.orgDb.query(
        'INSERT INTO payroll_snapshots(id,payroll_run_id,employee_id,payload,gross_amount,deductions,net_amount) VALUES(?,?,?,?,?,?,?)',
        {
          replacements: [
            uuid(),
            runId,
      ...
      await req.orgDb.query(
        'INSERT INTO payroll_payslips(id,payroll_run_id,employee_id,payslip_number,payload,gross_amount,deductions,net_amount) VALUES(?,?,?,?,?,?,?,?) ON DUPLICATE KEY UPDATE payload=VALUES(payload),gross_amount=VALUES(gross_amount),deductions=VALUES(deductions),net_amount=VALUES(net_amount)',
        {
          replacements: [
            ...
            runId,
            item.employee_id,
            `PAYSLIP-${runId}-${item.employee_id}`,
            JSON.stringify(payload),
            item.gross_amount,
            item.deductions,
            item.net_amount,
    ...
    await req.orgDb.query(
      'UPDATE payroll_runs SET status="processed",processed_by=? WHERE id=?',
      { replacements: [req.user?.sub || null, runId], transaction: tx },
    );
    await tx.commit();
    return { id: runId, status: 'processed', snapshot_count: items.length };
  } catch (e) {
    await tx.rollback();
    throw e;
  }
...
async function report(req, key, params) {
  ...
  let sql = `${def.sql} LIMIT ${limit} OFFSET ${offset}`;
  const replacements = def.sql.includes('?') ? [from, to] : [];
  if (params.employee_id && ['attendance', 'leave', 'payroll'].includes(key)) {
    const column = key === 'payroll' ? 'pp.employee_id' : 'employee_id';
    sql = sql.replace(
      / LIMIT \d+ OFFSET \d+$/,
      ` AND ${column}=? LIMIT ${limit} OFFSET ${offset}`,
    );
    replacements.push(String(params.employee_id));
  }
  const [rows] = await req.orgDb.query(sql, { replacements });
  return { report: key, from, to, limit, offset, rows };
}
...
async function closePeriod(req, periodKey) {
  ...
  try {
    ...
    await req.orgDb.query(
      'UPDATE finance_periods SET status="closed",closed_by=?,closed_at=NOW() WHERE period_key=?',
      { replacements: [req.user?.sub || null, periodKey], transaction: tx },
    );
    await tx.commit();
  ...
  try {
    ...
    await req.orgDb.query(
      'INSERT INTO finance_journals(id,journal_number,journal_date,narration,status,total_debit,created_by) VALUES(?,?,?,?,?,?,?)',
      {
        replacements: [
          reversalId,
          `REV-${rows[0].journal_number}`,
          new Date().toISOString().slice(0, 10),
          `Reversal of ${rows[0].journal_number}`,
          'posted',
          rows[0].total_debit,
          req.user?.sub || null,
        ],
...
async function createFinanceDocument(req, body) {
  ...
  try {
    ...
    const id = uuid();
    await req.orgDb.query(
      'INSERT INTO finance_documents(id,document_type,document_number,party_id,document_date,amount,status,due_date,taxable_amount,cgst,sgst,igst) VALUES(?,?,?,?,?,?,?,?,?,?,?,?)',
      {
        replacements: [
          id,
          body.document_type,
          ...
          'open',
          body.due_date || null,
          body.taxable_amount ?? body.amount,
          body.cgst || 0,
          body.sgst || 0,
          body.igst || 0,
        ],
...
async function reconcileBank(req, id) {
  ...
  try {
    ...
    const result = await postBankEffect(
      req.orgDb,
      id,
      {
        amount: rows[0].amount,
        direction: rows[0].direction,
        accountCode: rows[0].account_code || '1110',
        userId: req.user?.sub,
        date: rows[0].transaction_date,
        narration: rows[0].reference,
      },
      tx,
    );
    await req.orgDb.query(
      'UPDATE bank_transactions SET status="reconciled",journal_id=?,reconciled_at=NOW() WHERE id=?',
      { replacements: [result.journal_id, id], transaction: tx },
    );
    await tx.commit();
    return { id, status: 'reconciled', journal_id: result.journal_id };
  } catch (e) {
    await tx.rollback();
    throw e;
  }
...
async function createBankTransaction(req, body = {}) {
  ...
  try {
    const id = uuid();
    await req.orgDb.query(
      'INSERT INTO bank_transactions(id,account_id,transaction_date,reference,amount,direction,status,contra_account_id) VALUES(?,?,?,?,?,?,?,?)',
      {
        replacements: [
          id,
          body.account_id || '1110',
          body.transaction_date,
          body.reference || null,
          amount,
          body.direction,
          'unreconciled',
          body.contra_account_id || null,
        ],
        transaction: tx,
      },
    );
    const result = await postBankEffect(
      ...
      id,
      {
        ...
        accountCode: body.account_code || '1110',
        contraAccountCode: body.contra_account_code || '1100',
        userId: req.user?.sub,
        date: body.transaction_date,
        narration: body.narration || body.reference,
      },
      tx,
    );
    await req.orgDb.query(
...
async function transitionExpense(req, id, action) {
  ...
  try {
    ...
    if (action === 'approve') {
      await req.orgDb.query(
        'INSERT INTO expense_approvals(id,document_id,status,requested_by,decided_by,decided_at) VALUES(?,?,?,?,?,NOW()) ON DUPLICATE KEY UPDATE status="approved",decided_by=VALUES(decided_by),decided_at=NOW()',
        {
          replacements: [
            uuid(),
            id,
      ...
      await req.orgDb.query(
        'UPDATE finance_documents SET status="approved",approved_by=?,approved_at=NOW() WHERE id=?',
        { replacements: [req.user?.sub || null, id], transaction: tx },
      );
    } else if (action === 'reject') {
      await req.orgDb.query(
        'UPDATE expense_approvals SET status="rejected",decided_by=?,decided_at=NOW() WHERE document_id=?',
        { replacements: [req.user?.sub || null, id], transaction: tx },
      );
      await req.orgDb.query(
        'UPDATE finance_documents SET status="rejected" WHERE id=?',
        { replacements: [id], transaction: tx },
      );
    } else {
      ...
      const result = await postVendorPaymentEffect(
        ...
        id,
        id,
        Number(docs[0].amount),
        'bank',
        req.user?.sub,
        tx,
      );
      await req.orgDb.query(
        'UPDATE expense_approvals SET status="posted" WHERE document_id=?',
        { replacements: [id], transaction: tx },
      );
      await req.orgDb.query(
        'UPDATE finance_documents SET status="posted",posted_at=NOW() WHERE id=?',
        { replacements: [id], transaction: tx },
      );
      await tx.commit();
      return { id, status: 'posted', journal_id: result.journal_id };
    }
    await tx.commit();
...
async function recordVendorPayment(req, documentId, body = {}) {
  ...
  try {
    ...
    await req.orgDb.query(
      'UPDATE finance_documents SET paid_amount=?, status=CASE WHEN ?>=amount THEN "paid" ELSE "part_paid" END WHERE id=?',
      {
        replacements: [newPaidAmount, newPaidAmount, documentId],
        transaction: tx,
      },
    );
    const response = {
    ...
    await req.orgDb.query(
      ...
      {
        replacements: [uuid(), operationKey, JSON.stringify(response)],
        transaction: tx,
      },
    );
    await tx.commit();
...
async function recordPayrollOvertime(req, body = {}) {
  if (
    ...
    throw error(
      'payroll_run_id, employee_id, overtime_date, positive hours and hourly_rate are required',
    );
  }
  const id = uuid();
  const amount =
    Math.round(Number(body.hours) * Number(body.hourly_rate) * 100) / 100;
  await req.orgDb.query(
...
async function calculatePayroll(req, runId) {
  const tx = await req.orgDb.transaction();
  try {
    const [runs] = await req.orgDb.query(
      'SELECT * FROM payroll_runs WHERE id=? FOR UPDATE',
      { replacements: [runId], transaction: tx },
    );
    ...
    for (const employee of employees) {
      const [attendance] = await req.orgDb.query(
        ...
        {
          replacements: [employee.id, runs[0].period_start, runs[0].period_end],
          transaction: tx,
        },
      );
      const payload = payrollSnapshot(
        employee,
        {
          annual_ctc: employee.annual_ctc,
          components:
            typeof employee.components === 'string'
              ? JSON.parse(employee.components || '{}')
              : employee.components || {},
        },
        attendance[0] || {},
      );
      const [componentRows] = await req.orgDb.query(
        ...
        {
          replacements: [employee.assignment_id, runs[0].period_end],
          transaction: tx,
        },
      );
      const formulaValues = {
        basic: Number(payload.components.basic || payload.gross || 0),
        gross: Number(payload.gross || 0),
        annual_ctc: Number(employee.annual_ctc || 0),
        working_days: Number(attendance[0]?.working_days || 26),
      };
      ...
      payload.overtime = {
        hours: Number(overtimeRows[0]?.overtime_hours || 0),
        amount: overtime,
      };
    ...
    await req.orgDb.query(
      'UPDATE payroll_runs SET total_amount=?,status="calculated" WHERE id=?',
      { replacements: [total, runId], transaction: tx },
    );
    await tx.commit();
    return {
      id: runId,
...
async function transitionPayroll(req, runId, transition) {
  ...
  try {
    ...
    await req.orgDb.query(
      `UPDATE payroll_runs SET status=?,${fields} WHERE id=?`,
      { replacements: [status, actor, runId], transaction: tx },
    );
    await req.orgDb.query(
      'INSERT INTO activity_log(id,user_id,module,action,reference_type,reference_id,changes,ip_address) VALUES(?,?,?,?,?,?,?,?)',
      {
        replacements: [
          ...
          'payroll_run',
          runId,
          JSON.stringify({ from: rows[0].status, to: status }),
          req.ip || null,
        ],
        transaction: tx,
      },
...
async function createAccount(req, body) {
  ...
  const id = uuid();
  await req.orgDb.query(
    'INSERT INTO finance_accounts(id,code,name,account_type,parent_id,normal_balance) VALUES(?,?,?,?,?,?)',
    {
      replacements: [
        id,
        body.code,
...
async function submitJournal(req, id, action) {
  ...
  try {
    ...
    const actor = req.user?.sub || null;
    await req.orgDb.query(
      `UPDATE finance_journals SET status=?,${action === 'submit' ? 'submitted_by=?,submitted_at=NOW()' : action === 'approve' ? 'approved_by=?,approved_at=NOW()' : 'posted_at=NOW()'} WHERE id=?`,
      {
        replacements: action === 'post' ? [next, id] : [next, actor, id],
        transaction: tx,
      },
    );
    await req.orgDb.query(
      'INSERT INTO activity_log(id,user_id,module,action,reference_type,reference_id,changes,ip_address) VALUES(?,?,?,?,?,?,?,?)',
      {
        replacements: [
          ...
          actor,
          'finance',
          `finance.journal.${action}`,
          'journal',
          id,
          JSON.stringify({ from: rows[0].status, to: next }),
          req.ip || null,
        ],
        transaction: tx,
      },
...
async function financialStatement(req, type, params = {}) {
  ...
  const mapped = rows.map((r) => {
    ...
    return {
      ...
      debit,
      credit,
      balance: ['asset', 'expense'].includes(
  ...
  if (type === 'receivables' || type === 'payables') {
    ...
    const where =
      type === 'receivables'
        ? 'balance_amount > 0'
        : 'document_type="payable" AND status NOT IN ("paid","cancelled")';
    const [open] = await req.orgDb.query(
      `SELECT * FROM ${table} WHERE ${where} AND ${type === 'receivables' ? 'invoice_date' : 'document_date'} BETWEEN ? AND ? ORDER BY ${type === 'receivables' ? 'invoice_date' : 'document_date'}`,
      { replacements: [from, to] },
    );
    return {
      statement: type,
      ...
      rows: open.map((r) => ({
        ...
        ageing_days: Math.max(
          ...
          Math.floor(
            (Date.now() -
              new Date(
                r.due_date || r.invoice_date || r.document_date,
              ).getTime()) /
              86400000,
          ),
        ),
  ...
  return {
    ...
    to,
    rows:
      type === 'income_statement' || type === 'profit_loss'
        ? mapped.filter((r) =>
            ['income', 'revenue', 'expense', 'cost'].includes(
              String(r.account_type).toLowerCase(),
            ),
          )
        : mapped,
...
async function gstSnapshot(req, body) {
  ...
  try {
    const [existing] = await req.orgDb.query(
      'SELECT id,calculation FROM gst_context_snapshots WHERE source_type=? AND source_id=? FOR UPDATE',
      { replacements: [body.source_type, body.source_id], transaction: tx },
    );
    if (existing[0]) {
      await tx.commit();
      return {
        id: existing[0].id,
        calculation:
          typeof existing[0].calculation === 'string'
            ? JSON.parse(existing[0].calculation)
            : existing[0].calculation,
        already_exists: true,
      };
    }
    const id = uuid();
    await req.orgDb.query(
      'INSERT INTO gst_context_snapshots(id,source_type,source_id,context,calculation) VALUES(?,?,?,?,?)',
      {
        replacements: [
          ...
          body.source_id,
          JSON.stringify({
            org_state: body.org_state || req.org.state,
    ...
    await req.orgDb.query(
      'INSERT INTO gst_snapshots(id,source_type,source_id,calculation,taxable_amount,cgst,sgst,igst) VALUES(?,?,?,?,?,?,?,?)',
      {
        replacements: [
          uuid(),
          body.source_type,
          body.source_id,
          JSON.stringify(calculation),
          calculation.totals.taxable,
          calculation.totals.cgst,
          calculation.totals.sgst,
          calculation.totals.igst,
        ],
        transaction: tx,
      },
    );
...
async function postPayrollToFinance(req, runId, body = {}) {
  ...
  try {
    ...
    if (posted[0]) {
      ...
      return {
        run_id: runId,
        journal_id: posted[0].journal_id,
        already_posted: true,
      };
    }
    const [accounts] = await req.orgDb.query(
      'SELECT id,code FROM finance_accounts WHERE code IN (?,?)',
      {
        replacements: [
          body.expense_account_code || 'SALARY_EXPENSE',
          body.payable_account_code || 'SALARY_PAYABLE',
        ],
        transaction: tx,
      },
    );
    const expense = accounts.find(
      (a) => a.code === (body.expense_account_code || 'SALARY_EXPENSE'),
    );
    ...
    const number = `PAYROLL-${runId}`;
    await req.orgDb.query(
      'INSERT INTO finance_journals(id,journal_number,journal_date,narration,status,total_debit,created_by,approved_by,approved_at,posted_at) VALUES(?,?,?,?,?,?,?,?,NOW(),NOW())',
      {
        replacements: [
          journalId,
          number,
          runs[0].period_end || new Date().toISOString().slice(0, 10),
          `Payroll ${runs[0].period}`,
          'posted',
          sum[0].gross,
          req.user?.sub || null,
          req.user?.sub || null,
    ...
    await req.orgDb.query(
      'INSERT INTO payroll_finance_posts(id,payroll_run_id,journal_id) VALUES(?,?,?)',
      { replacements: [uuid(), runId, journalId], transaction: tx },
    );
    await req.orgDb.query(
      'INSERT INTO activity_log(id,user_id,module,action,reference_type,reference_id,changes,ip_address) VALUES(?,?,?,?,?,?,?,?)',
      {
        replacements: [
          ...
          req.user?.sub || null,
          'payroll',
          'payroll.post_to_finance',
          'payroll_run',
          runId,
          JSON.stringify({
            journal_id: journalId,
            amount: Number(sum[0].gross),
          }),
          req.ip || null,
        ],
        transaction: tx,
...
async function createQualitySpecification(req, body = {}) {
  ...
  const result = await withIdempotency(
    req,
    'quality.specification.create',
    async () => {
      await req.orgDb.query(
        'INSERT INTO quality_specifications(id,code,name,specification_type,version_no,effective_from,status,specification,approved_by,approved_at,created_by) VALUES(?,?,?,?,?,?,?,?,?,?,?)',
        {
          replacements: [
            id,
            record.code,
            record.name,
            record.specification_type,
            record.version_no,
            record.effective_from,
            record.status,
            JSON.stringify(record.specification),
...
async function createQualityParameter(req, body = {}) {
  ...
  const result = await withIdempotency(
    req,
    'quality.parameter.create',
    async () => {
      await req.orgDb.query(
        'INSERT INTO quality_parameters(id,specification_id,parameter_code,parameter_name,target_value,upper_limit,lower_limit,unit,is_active) VALUES(?,?,?,?,?,?,?,?,?)',
        {
          replacements: [
            id,
...
async function createQualitySamplingPlan(req, body = {}) {
  ...
  const result = await withIdempotency(
    ...
    async () => {
      await req.orgDb.query(
        'INSERT INTO quality_sampling_plans(id,specification_id,plan_code,sample_size,sampling_method,acceptance_level,is_active) VALUES(?,?,?,?,?,?,?)',
        {
          replacements: [
            id,
            record.specification_id,
            record.plan_code,
            record.sample_size,
            record.sampling_method || 'random',
            record.acceptance_level || null,
            record.is_active,
          ],
        },
      );
...
async function createQualityAcceptanceCriteria(req, body = {}) {
  ...
  const result = await withIdempotency(
    req,
    'quality.acceptance_criteria.create',
    async () => {
      await req.orgDb.query(
        'INSERT INTO quality_acceptance_criteria(id,specification_id,criterion_code,criterion_name,expression,pass_threshold,status) VALUES(?,?,?,?,?,?,?)',
        {
          replacements: [
            id,
            record.specification_id,
...
async function createInspectionSnapshot(req, inspectionId, payload = {}) {
  ...
  try {
    const [rows] = await req.orgDb.query(
      'SELECT COALESCE(MAX(snapshot_version),0) + 1 AS next_version FROM inspection_snapshots WHERE inspection_id=? FOR UPDATE',
    ...
    await req.orgDb.query(
      'INSERT INTO inspection_snapshots(id,inspection_id,snapshot_version,payload,recorded_by) VALUES(?,?,?,?,?)',
      {
        replacements: [
          id,
          inspectionId,
...
async function createDepartment(req, body = {}) {
  ...
  const result = await withIdempotency(
    ...
    async () => {
      await req.orgDb.query(
        'INSERT INTO hr_departments(id,code,name,parent_id,location_id,is_active) VALUES(?,?,?,?,?,?)',
        {
          replacements: [
            id,
            body.code,
...
async function createHrMaster(req, kind, body = {}) {
  const definitions = {
    ...
    document_type: [
      'hr_document_types',
      'code,name,document_sensitive,is_active',
    ],
  };
  ...
  return withIdempotency(req, `hr.${kind}.create`, async () => {
    ...
    await req.orgDb.query(
      `INSERT INTO ${def[0]}(id,${def[1]}) VALUES(?,${values
        .slice(1)
        .map(() => '?')
        .join(',')})`,
      { replacements: values },
...
async function createLocation(req, body = {}) {
  ...
  const result = await withIdempotency(req, 'hr.location.create', async () => {
    await req.orgDb.query(
      'INSERT INTO hr_locations(id,code,name,address,parent_id,is_active) VALUES(?,?,?,?,?,?)',
      {
        replacements: [
          id,
          body.code,
...
async function createGrade(req, body = {}) {
  ...
  const result = await withIdempotency(req, 'hr.grade.create', async () => {
    await req.orgDb.query(
      'INSERT INTO hr_grades(id,code,name,pay_band,is_active) VALUES(?,?,?,?,?)',
      {
        replacements: [
          id,
          body.code,
    ...
    return {
      ...
      code: body.code,
      name: body.name,
      pay_band: Number(body.pay_band || 0),
      is_active: body.is_active !== false ? 1 : 0,
    };
  });
  return result;
...
async function createShift(req, body = {}) {
  ...
  const result = await withIdempotency(req, 'hr.shift.create', async () => {
    await req.orgDb.query(
      'INSERT INTO hr_shifts(id,code,name,start_time,end_time,work_hours,is_active) VALUES(?,?,?,?,?,?,?)',
      {
        replacements: [
          id,
          body.code,
    ...
    return {
      ...
      work_hours: Number(body.work_hours || 0),
      is_active: body.is_active !== false ? 1 : 0,
    };
  });
  return result;
...
async function createCalendar(req, body = {}) {
  ...
  const result = await withIdempotency(req, 'hr.calendar.create', async () => {
    await req.orgDb.query(
      'INSERT INTO hr_calendars(id,code,year,calendar_type,metadata,is_active) VALUES(?,?,?,?,?,?)',
      {
        replacements: [
          id,
          body.code,
          Number(body.year),
          body.calendar_type || 'annual',
    ...
    return {
      ...
      calendar_type: body.calendar_type || 'annual',
...
async function createAttendanceCorrection(req, body = {}) {
  ...
  const record = {
    ...
    id,
    status: 'pending',
    original_status: body.original_status || 'present',
    reason: body.reason || null,
    requested_by: req.user?.sub || body.requested_by || null,
  };
  const result = await withIdempotency(
    req,
    ...
    async () => {
      await req.orgDb.query(
        'INSERT INTO attendance_corrections(id,employee_id,attendance_date,original_status,proposed_status,reason,requested_by,status) VALUES(?,?,?,?,?,?,?,?)',
        {
          replacements: [
            id,
            record.employee_id,
...
async function createHoliday(req, body = {}) {
  ...
  const result = await withIdempotency(req, 'hr.holiday.create', async () => {
    await req.orgDb.query(
      'INSERT INTO leave_holidays(id,holiday_date,holiday_name,is_recurring) VALUES(?,?,?,?)',
      {
        replacements: [
          id,
          body.holiday_date,
...
async function createEmployeeHistory(req, body = {}) {
  ...
  const result = await withIdempotency(
    req,
    'hr.employee_history.create',
    async () => {
      await req.orgDb.query(
        'INSERT INTO employee_history(id,employee_id,effective_date,field_name,old_value,new_value,changed_by) VALUES(?,?,?,?,?,?,?)',
        {
          replacements: [
            id,
            body.employee_id,
            ...
            body.old_value || null,
            body.new_value || null,
            req.user?.sub || body.changed_by || null,
          ],
        },
      );
      await recordAudit(
...
async function getEmployee360(req, employeeId) {
  ...
  const [deptRows] = employee.department_id
    ? await req.orgDb.query('SELECT * FROM hr_departments WHERE id=? LIMIT 1', {
        replacements: [employee.department_id],
      })
    : [[]];
  const [locRows] = employee.location_id
    ? await req.orgDb.query('SELECT * FROM hr_locations WHERE id=? LIMIT 1', {
        replacements: [employee.location_id],
      })
    : [[]];
  const [gradeRows] = employee.grade_id
    ? await req.orgDb.query('SELECT * FROM hr_grades WHERE id=? LIMIT 1', {
        replacements: [employee.grade_id],
      })
    : [[]];
  const [shiftRows] = employee.shift_id
    ? await req.orgDb.query('SELECT * FROM hr_shifts WHERE id=? LIMIT 1', {
        replacements: [employee.shift_id],
      })
    : [[]];
  const [attendanceRows] = await req.orgDb.query(
    'SELECT * FROM attendance WHERE employee_id=? ORDER BY attendance_date DESC LIMIT 30',
    { replacements: [employeeId] },
  );
  const [leaveRows] = await req.orgDb.query(
    'SELECT * FROM leave_balances WHERE employee_id=? ORDER BY effective_date DESC LIMIT 20',
    { replacements: [employeeId] },
  );
  const privileged = [
    'admin',
...
async function approveAttendanceCorrection(req, id, body = {}) {
  ...
  try {
    const [rows] = await req.orgDb.query(
      'SELECT * FROM attendance_corrections WHERE id=? FOR UPDATE',
      { replacements: [id], transaction: tx },
    );
    const row = rows[0];
    ...
    if (status === 'approved') {
      await req.orgDb.query(
        'UPDATE attendance SET status=?,corrected_by=?,approved_by=?,approved_at=NOW(),correction_status=? WHERE employee_id=? AND attendance_date=?',
        {
          replacements: [
            row.proposed_status,
...
async function createLeavePolicy(req, body = {}) {
  ...
  const result = await withIdempotency(
    req,
    'hr.leave_policy.create',
    async () => {
      await req.orgDb.query(
        'INSERT INTO leave_policies(id,employee_id,leave_type,annual_days,carry_forward_days,max_accumulation,is_default,effective_from) VALUES(?,?,?,?,?,?,?,?)',
        {
          replacements: [
            id,
            body.employee_id || null,
            body.leave_type,
            Number(body.annual_days || 0),
            Number(body.carry_forward_days || 0),
            Number(body.max_accumulation || 0),
            body.is_default ? 1 : 0,
            body.effective_from,
          ],
        },
      ...
      return {
        ...
        max_accumulation: Number(body.max_accumulation || 0),
      };
    },
  );
  return result;
...
async function createLeaveAccrualRule(req, body = {}) {
  ...
  const result = await withIdempotency(
    req,
    'hr.leave_accrual.create',
    async () => {
      await req.orgDb.query(
        'INSERT INTO leave_accrual_rules(id,leave_type,accrual_period,accrual_days,max_balance,effective_from,is_active) VALUES(?,?,?,?,?,?,?)',
        {
          replacements: [
            id,
            body.leave_type,
            body.accrual_period,
            Number(body.accrual_days || 0),
            Number(body.max_balance || 0),
            body.effective_from,
            body.is_active !== false ? 1 : 0,
          ],
        },
      ...
      return {
        id,
        ...body,
        accrual_days: Number(body.accrual_days || 0),
        max_balance: Number(body.max_balance || 0),
      };
    },
  );
  return result;
...
async function approveLeaveRequest(req, id, body = {}) {
  ...
  try {
    ...
    await req.orgDb.query(
      'UPDATE leave_requests SET status=?,approved_by=?,approved_at=NOW(),days=? WHERE id=?',
      {
        replacements: [status, req.user?.sub || null, days, id],
        transaction: tx,
      },
    );
    if (status === 'approved' && days > 0) {
      const [balanceRows] = await req.orgDb.query(
        'SELECT * FROM leave_balances WHERE employee_id=? AND leave_type=? ORDER BY effective_date DESC LIMIT 1 FOR UPDATE',
        {
          replacements: [leave.employee_id, leave.leave_type || 'annual'],
          transaction: tx,
        },
      );
    ...
    await recordAudit(req, 'hr', 'hr.leave.approval', 'leave_request', id, {
      status,
      days,
    });
    await tx.commit();
...
async function validateLeaveRequest(req, body = {}) {
  ...
  if (existing[0]) {
    throw Object.assign(
      new Error(
        'Leave request overlaps with an existing approved or pending period',
      ),
      { status: 409, code: 'LEAVE_OVERLAP' },
    );
  }

  const [holidayRows] = await req.orgDb.query(
    'SELECT holiday_date FROM leave_holidays WHERE holiday_date BETWEEN ? AND ?',
    { replacements: [body.from_date, body.to_date] },
  );
  ...
  const balance = await getLeaveBalanceForEmployee(
    req,
    body.employee_id,
    body.leave_type || 'annual',
  );
  if (Number(balance?.closing_balance || 0) < businessDays) {
    throw Object.assign(
      new Error('Leave balance is insufficient for the requested duration'),
  ...
  return {
    ...
    balance: Number(balance?.closing_balance || 0),
    overlaps: [],
    holiday_count: holidayRows.length,
  };
}

async function getLeaveBalanceForEmployee(
  req,
  employeeId,
  leaveType = 'annual',
) {
  const [rows] = await req.orgDb.query(
    'SELECT * FROM leave_balances WHERE employee_id=? AND leave_type=? ORDER BY effective_date DESC LIMIT 1',
    { replacements: [employeeId, leaveType] },
  );
  return (
    rows[0] || {
...
async function recordLeaveBalanceTransaction(req, body = {}) {
  ...
  try {
    const [balances] = await req.orgDb.query(
      'SELECT * FROM leave_balances WHERE employee_id=? AND leave_type=? ORDER BY effective_date DESC LIMIT 1 FOR UPDATE',
      { replacements: [body.employee_id, body.leave_type], transaction: tx },
    );
    const current = Number(balances[0]?.closing_balance || 0);
    const delta = Number(body.quantity || 0);
    const next = ['credit', 'accrual', 'carry_forward'].includes(
      body.transaction_type,
    )
      ? current + delta
      : current - delta;
    ...
    await req.orgDb.query(
      'INSERT INTO leave_balances(id,employee_id,leave_type,opening_balance,accrued_days,used_days,carry_forward_days,closing_balance,effective_date) VALUES(?,?,?,?,?,?,?,?,?) ON DUPLICATE KEY UPDATE opening_balance=VALUES(opening_balance),accrued_days=VALUES(accrued_days),used_days=VALUES(used_days),carry_forward_days=VALUES(carry_forward_days),closing_balance=VALUES(closing_balance),effective_date=VALUES(effective_date)',
      {
        replacements: [
          ...
          body.leave_type,
          current,
          body.transaction_type === 'accrual' ? delta : 0,
          body.transaction_type === 'usage' ? delta : 0,
          body.transaction_type === 'carry_forward' ? delta : 0,
          next,
          new Date().toISOString().slice(0, 10),
        ],
        transaction: tx,
...
async function transitionNcr(req, id, body = {}) {
  ...
  return withIdempotency(
    req,
    `quality.ncr.transition.${effectKey}`,
    async () => {
      const tx = await req.orgDb.transaction();
      try {
        const [rows] = await req.orgDb.query(
        ...
        if (
          ...
          !(
            body.containment_action ||
            rows[0].containment_action ||
        ...
        await req.orgDb.query(
          'UPDATE quality_ncrs SET status=?,root_cause=COALESCE(?,root_cause),containment_action=COALESCE(?,containment_action),corrective_action=COALESCE(?,corrective_action),verified_by=?,verified_at=?,closed_by=?,closed_at=? WHERE id=?',
          {
            replacements: [
              ...
              status === 'verification' ? req.user?.sub || null : null,
              status === 'verification' ? new Date() : null,
              status === 'closed' ? req.user?.sub || null : null,
        ...
        await req.orgDb.query(
          'INSERT INTO quality_ncr_events(id,ncr_id,event_type,actor_id,event_note) VALUES(?,?,?,?,?)',
          {
            replacements: [
              ...
              'status_change',
              req.user?.sub || null,
              `NCR transitioned to ${status}`,
            ],
            transaction: tx,
          },
        );
        const [nextRows] = await req.orgDb.query(
          'SELECT * FROM quality_ncrs WHERE id=?',
          { replacements: [id], transaction: tx },
        );
        await recordAudit(req, 'quality', 'quality.ncr.transition', 'ncr', id, {
          status,
...
async function applyDispositionStockEffect(
  req,
  tx,
  inspectionId,
  disposition,
  ...
  if (inspection.inspection_type === 'incoming') {
    const [[legacy]] = await req.orgDb.query(
      ...
      {
        replacements: [
          inspection.reference_id || inspection.source_id,
          targetItemId,
        ],
        transaction: tx,
      },
  ...
  const [summaryRows] = await req.orgDb.query(
    ...
    { replacements: [targetItemId, targetWarehouseId], transaction: tx },
  );
  ...
  if (summaryRows[0]) {
    await req.orgDb.query(
      'UPDATE stock_summary SET current_qty=?,total_value=?,last_updated=NOW() WHERE item_id=? AND warehouse_id=?',
      {
        replacements: [
          nextQty,
          nextQty * Number(summaryRows[0].avg_rate || 0),
          targetItemId,
          targetWarehouseId,
        ],
        transaction: tx,
    ...
    await req.orgDb.query(
      'INSERT INTO stock_summary(item_id,warehouse_id,current_qty,avg_rate,total_value,last_updated) VALUES(?,?,?,0,0,NOW())',
      {
        replacements: [targetItemId, targetWarehouseId || '', nextQty],
        transaction: tx,
      },
    );
  }
...
async function disposeInspection(req, body) {
  ...
  return withIdempotency(req, `quality.disposition.${effectKey}`, async () => {
    ...
    try {
      const [existing] = await req.orgDb.query(
        'SELECT id,effect_key FROM quality_dispositions WHERE effect_key=?',
        { replacements: [effectKey], transaction: tx },
      );
      if (existing[0]) {
        await tx.commit();
      ...
      if (!warehouseId) {
        const [[setting]] = await req.orgDb.query(
          "SELECT setting_value FROM company_settings WHERE setting_key='quality.default_quarantine_warehouse'",
          { transaction: tx },
        );
        warehouseId = setting?.setting_value || null;
      }
      const id = require('uuid').v4();
      const dispositionResult = await applyDispositionStockEffect(
        req,
        tx,
        body.inspection_id,
        body.disposition,
        body.quantity,
        warehouseId,
        inspection.item_id || body.item_id || null,
      );
      await req.orgDb.query(
        'INSERT INTO quality_dispositions(id,inspection_id,disposition,quantity,warehouse_id,batch_id,serial_id,effect_key,created_by) VALUES(?,?,?,?,?,?,?,?,?)',
        {
          replacements: [
            id,
            body.inspection_id,
      ...
      await req.orgDb.query(
        'INSERT INTO quality_disposition_effects(id,disposition_id,item_id,warehouse_id,disposition,quantity,movement_type,source_reference,created_by) VALUES(?,?,?,?,?,?,?,?,?)',
        {
          replacements: [
            require('uuid').v4(),
            id,
            ...
            body.disposition,
            body.quantity,
            dispositionResult.movement_type,
            body.disposition,
            req.user?.sub || null,
          ],
          transaction: tx,
      ...
      await req.orgDb.query(
        'UPDATE qc_inspections SET status=?,result=? WHERE id=?',
        {
          replacements: [nextStatus, body.disposition, body.inspection_id],
          transaction: tx,
        },
      );
      await recordAudit(
        ...
        id,
        {
          ...
          stock_effect: dispositionResult,
        },
      );
      await tx.commit();
      return {
...
module.exports = {
  ...
  createQualityAcceptanceCriteria,
