...
async function recordInvoicePayment(db, invoiceId, amount, details, userId) {
  ...
  try {
    ...
    await db.query('INSERT INTO invoice_payments(id,invoice_id,amount,method,reference,created_by) VALUES(?,?,?,?,?,?)', { replacements: [uuid(), invoiceId, amount, details?.method || 'bank', details?.reference || null, userId], transaction: tx });
    ...
    await tx.commit(); return { invoice_id: invoiceId, paid: Number(amount), balance_amount: next };
  } catch (e) { await tx.rollback(); throw e; }
}
