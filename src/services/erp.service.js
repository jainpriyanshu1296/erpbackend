...
async function nextNumber(
  ...
  const [rows] = await db.query(
    'SELECT * FROM number_series WHERE series_key=? FOR UPDATE',
...
async function postStockAdjustment(db, input, userId) {
  ...
  try {
    ...
    for (const item of input.items) {
      ...
      await db.query(
        'INSERT INTO stock_summary(item_id,warehouse_id,current_qty,avg_rate,total_value) VALUES(?,?,?,?,?) ON DUPLICATE KEY UPDATE current_qty=?, total_value=?',
        {
          replacements: [
            ...
            input.warehouse_id,
            next,
            item.rate || summary[0]?.avg_rate || 0,
            next * Number(item.rate || summary[0]?.avg_rate || 0),
            next,
            next * Number(item.rate || summary[0]?.avg_rate || 0),
          ],
          transaction: tx,
      ...
      await db.query(
        'INSERT INTO stock_ledger(id,item_id,warehouse_id,transaction_type,reference_type,reference_id,qty_in,qty_out,balance_qty,rate,amount,created_by) VALUES(?,?,?,?,?,?,?,?,?,?,?,?)',
        {
          replacements: [
            uuid(),
            item.item_id,
    ...
    await db.query(
      'UPDATE stock_adjustments SET status="posted",posted_at=NOW() WHERE id=?',
      { replacements: [id], transaction: tx },
    );
    await tx.commit();
    return { id, adjustment_number: number, status: 'posted' };
  } catch (e) {
    await tx.rollback();
    throw e;
  }
...
async function recordInvoicePayment(db, invoiceId, amount, details, userId) {
  ...
  try {
    ...
    await db.query('UPDATE invoices SET balance_amount=?,status=? WHERE id=?', {
      replacements: [next, next === 0 ? 'paid' : 'part_paid', invoiceId],
      transaction: tx,
    });
    const result = {
...
function calculatePayroll(employee, attendance = {}, deductions = {}) {
  const monthly =
    Number(employee.basic_salary || 0) + Number(employee.allowances || 0);
