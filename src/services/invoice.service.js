  ...
  try {
    ...
    if (soId) {
      ...
      const [sent] = await query(
        "SELECT dci.item_id,SUM(dci.quantity) quantity FROM delivery_challan_items dci JOIN delivery_challans dc ON dc.id=dci.challan_id WHERE dc.so_id=? AND dc.status IN ('dispatched','delivered') GROUP BY dci.item_id",
        [soId],
      );
    ...
    const fingerprint = createHash('sha256')
      ...
      .digest('hex');
    const [[existing]] = await query(
      'SELECT * FROM invoices WHERE creation_key=? LIMIT 1',
      [key],
    );
    if (existing) {
    ...
    const priced = priceLines(
      ...
      String(company.setting_value).trim().toLowerCase() !==
        String(customer.state).trim().toLowerCase(),
    );
