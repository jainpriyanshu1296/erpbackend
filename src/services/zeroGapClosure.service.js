...
async function write(db, table, data, userId, operationKey) {
  ...
  if (initial[table]) {
    assert(
      !data.status || data.status === initial[table],
      'Records must be created in their initial state',
    );
    data.status = initial[table];
  }
  if (table === 'batch') {
    assert(
      data.quantity === undefined || Number(data.quantity) === 0,
      'Batch quantities are maintained only by stock movements',
    );
    data.quantity = 0;
  ...
  try {
    ...
    if (table === 'rfqSupplier') {
      const [[rfq]] = await db.query(
        ...
        { replacements: [data.rfq_id || null], transaction: tx },
      );
      const [[vendor]] = await db.query(
        'SELECT id FROM vendors WHERE id=? AND is_active=1',
        { replacements: [data.supplier_id || null], transaction: tx },
      );
      assert(
        rfq && ['draft', 'requested'].includes(rfq.status) && vendor,
        'Choose an active vendor and open RFQ',
      );
      data.status = 'invited';
    }
    if (table === 'quotationLine') {
      const [[source]] = await db.query(
        'SELECT s.rfq_id,r.status FROM rfq_suppliers s JOIN rfqs r ON r.id=s.rfq_id WHERE s.id=? FOR UPDATE',
        { replacements: [data.rfq_supplier_id || null], transaction: tx },
      );
      assert(
        source && ['requested', 'quoted'].includes(source.status),
        'RFQ must be requested or quoted',
      );
      const [[item]] = await db.query(
        'SELECT quantity FROM rfq_items WHERE rfq_id=? AND item_id=?',
        {
          replacements: [source.rfq_id, data.item_id || null],
          transaction: tx,
        },
      );
      assert(
        item && Number(data.quantity) === Number(item.quantity),
        'Quote the requested item and quantity',
      );
      assert(
        Number.isFinite(Number(data.unit_price)) &&
          Number(data.unit_price) >= 0 &&
          Number.isFinite(Number(data.tax_rate || 0)) &&
          Number(data.tax_rate || 0) >= 0 &&
          Number(data.tax_rate || 0) <= 100,
        'Invalid price or tax rate',
      );
      data.is_selected = 0;
    }
    if (['batch', 'serial', 'countLine', 'output', 'scrap'].includes(table)) {
      const [[item]] = await db.query(
        ...
        { replacements: [data.item_id || null], transaction: tx },
      );
    ...
    if (data.warehouse_id) {
      const [[warehouse]] = await db.query(
        ...
        { replacements: [data.warehouse_id], transaction: tx },
      );
    ...
    if (['output', 'scrap', 'downtime'].includes(table)) {
      const [[order]] = await db.query(
        ...
        { replacements: [data.production_order_id || null], transaction: tx },
      );
      assert(
    ...
    if (table === 'countLine') {
      const [[count]] = await db.query(
        'SELECT * FROM physical_counts WHERE id=? FOR UPDATE',
        { replacements: [data.count_id || null], transaction: tx },
      );
      assert(
        count && ['draft', 'open'].includes(count.status),
        'Lines can only be recorded on a draft or open count',
      );
      assert(
        data.counted_qty !== undefined &&
          Number.isFinite(Number(data.counted_qty)) &&
          Number(data.counted_qty) >= 0,
        'Counted quantity must be non-negative',
      );
      const [[existing]] = await db.query(
        'SELECT id FROM physical_count_lines WHERE count_id=? AND item_id=?',
        { replacements: [data.count_id, data.item_id], transaction: tx },
      );
      ...
      const [[stock]] = await db.query(
        ...
        { replacements: [data.item_id, count.warehouse_id], transaction: tx },
      );
    ...
    await db.query(
      `INSERT INTO ${TABLES[table]} (id,${columns.join(',')}) VALUES (?,${columns.map(() => '?').join(',')})`,
    ...
    if (operationKey)
      await db.query(
        ...
        {
          replacements: [uuid(), operationKey, JSON.stringify(result)],
          transaction: tx,
        },
      );
    await tx.commit();
...
async function transition(db, table, id, status, userId) {
  ...
  const statusMap = {
    ...
    count: ['draft', 'open', 'submitted', 'approved', 'cancelled'],
    purchaseReturn: ['draft', 'cancelled'],
    salesReturn: ['draft', 'cancelled'],
    creditNote: ['draft', 'cancelled'],
  };
  assert(statusMap[table].includes(status), 'Invalid status');
  const tx = await db.transaction();
  try {
    const [rows] = await db.query(
      `SELECT status FROM ${TABLES[table]} WHERE id=? FOR UPDATE`,
    ...
    if (table === 'approval') {
      ...
      if (status === 'approved') {
        assert(
          steps.length === 0 ||
            steps.every((step) => step.status === 'approved'),
          'All approval steps must be approved first',
        );
      }
      await db.query(
        'UPDATE approval_requests SET decided_by=?,decided_at=NOW() WHERE id=?',
        { replacements: [userId || null, id], transaction: tx },
      );
    }
    await db.query(`UPDATE ${TABLES[table]} SET status=? WHERE id=?`, {
...
async function actOnApproval(db, approvalId, action, userId, role, notes) {
  ...
  try {
    const [rows] = await db.query(
      ...
      { replacements: [approvalId], transaction: tx },
    );
    ...
    if (current) {
      ...
      await db.query(
        'UPDATE approval_steps SET status=?,acted_by=?,acted_at=NOW() WHERE id=? AND status=?',
        {
          replacements: [action, userId || null, current.id, 'pending'],
          transaction: tx,
        },
      );
      await db.query(
        'INSERT INTO approval_step_events(id,approval_step_id,action,acted_by,notes) VALUES(?,?,?,?,?)',
        {
          replacements: [
            uuid(),
            current.id,
    ...
    const remainingPending = steps.filter(
      (step) => step.id !== current?.id && step.status === 'pending',
    ).length;
    const nextStatus =
      action === 'rejected'
        ? 'rejected'
        : remainingPending
          ...
          : 'approved';
    await db.query(
      'UPDATE approval_requests SET status=?,decided_by=?,decided_at=IF(? IN ("approved","rejected"),NOW(),decided_at) WHERE id=?',
      {
        replacements: [nextStatus, userId || null, nextStatus, approvalId],
        transaction: tx,
      },
    );
    await audit(
...
async function applyStockEffect(
  ...
  assert(
    operationKey && referenceType && referenceId && itemId && warehouseId,
    'Stock effect context is required',
  );
  ...
  try {
    const [existing] = await db.query(
      'SELECT id FROM stock_effects WHERE operation_key=? FOR UPDATE',
      { replacements: [operationKey], transaction: tx },
    );
    if (existing.length) {
    ...
    if (batchId) {
      const [batches] = await db.query(
        ...
        { replacements: [batchId], transaction: tx },
      );
      assert(
      ...
      await db.query(
        ...
        {
          replacements: [direction === 'in' ? qty : -qty, batchId],
          transaction: tx,
        },
      );
    }
    if (serialId) {
      ...
      const [serials] = await db.query(
        'SELECT item_id,status FROM stock_serials WHERE id=? FOR UPDATE',
        { replacements: [serialId], transaction: tx },
      );
      assert(
    ...
    const averageRate =
      direction === 'in' && next > 0
        ? totalValue / next
        : Number(stockRows[0]?.avg_rate || rate || 0);
    await db.query(
    ...
    await db.query(
      ...
      {
        replacements: [uuid(), operationKey, referenceType, referenceId],
        transaction: tx,
      },
    );
    if (ownsTransaction) await tx.commit();
...
async function allocatePayment(db, paymentId, invoiceId, amount, userId) {
  ...
  try {
    ...
    const [[allocated]] = await db.query(
      'SELECT COALESCE(SUM(allocated_amount),0) total FROM payment_allocations WHERE invoice_id=? FOR UPDATE',
      { replacements: [invoiceId], transaction: tx },
    );
    const [[paid]] = await db.query(
      'SELECT COALESCE(SUM(amount),0) total FROM invoice_payments WHERE invoice_id=? FOR UPDATE',
      { replacements: [invoiceId], transaction: tx },
    );
    const balance = Math.max(
      0,
      Number(invoices[0].total_amount || 0) -
        Number(paid.total || 0) -
    ...
    await db.query(
      'UPDATE invoices SET balance_amount=?,status=CASE WHEN ?=0 THEN "paid" WHEN ?<total_amount THEN "part_paid" ELSE status END WHERE id=?',
      {
        replacements: [remaining, remaining, remaining, invoiceId],
        transaction: tx,
      },
    );
    await audit(
      ...
      'invoice',
      invoiceId,
      { payment_id: paymentId, amount: value },
      tx,
    );
    await tx.commit();
    return {
...
async function postPhysicalCount(db, countId, userId) {
  ...
  try {
    const [counts] = await db.query(
      'SELECT * FROM physical_counts WHERE id=? FOR UPDATE',
      { replacements: [countId], transaction: tx },
    );
    assert(counts.length, 'Physical count not found');
    if (counts[0].status === 'posted') {
      await tx.commit();
      return { id: countId, status: 'posted', already_applied: true };
    }
    assert(
      counts[0].status === 'approved',
      'Count must be approved before posting',
    );
    const [lines] = await db.query(
      'SELECT * FROM physical_count_lines WHERE count_id=? FOR UPDATE',
      { replacements: [countId], transaction: tx },
    );
    assert(
      lines.length && lines.every((line) => line.counted_qty !== null),
      'All physical count lines must be counted',
    );
    for (const line of lines) {
      assert(
        Number.isFinite(Number(line.counted_qty)) &&
      ...
      const [[stock]] = await db.query(
        'SELECT current_qty FROM stock_summary WHERE item_id=? AND warehouse_id=? FOR UPDATE',
        {
          replacements: [line.item_id, counts[0].warehouse_id],
          transaction: tx,
        },
      );
      assert(
        Number(stock?.current_qty || 0) === Number(line.system_qty),
      ...
      await applyStockEffect(db, {
        ...
        referenceId: countId,
        itemId: line.item_id,
        warehouseId: counts[0].warehouse_id,
    ...
    await db.query(
      "UPDATE physical_counts SET status='posted',posted_at=NOW(),approved_by=COALESCE(approved_by,?) WHERE id=?",
      { replacements: [userId || null, countId], transaction: tx },
    );
    await audit(
      db,
      userId,
      'physical_count_posted',
      'physical_count',
      countId,
      { lines: lines.length },
      tx,
...
async function postProductionEffect(db, kind, id, userId, warehouseId) {
  if (kind === 'output') {
    ...
    const [[link]] = await db.query(
      ...
      { replacements: [output.production_order_id] },
    );
    assert(
      link,
      'Release the production order and issue its materials before posting output',
    );
    return require('./salesProduction.service').completeWorkOrder(
      db,
      link.target_id,
  ...
  try {
    ...
    await db
      .query(`UPDATE ${table} SET status='posted' WHERE id=?`, {
        replacements: [id],
        transaction: tx,
      })
      .catch(() => {});
...
async function issueCreditNote(db, id, userId) {
  ...
  try {
    ...
    assert(
      notes[0].invoice_id,
      'invoice_id is required before issuing a credit note',
    );
    const [invoices] = await db.query(
      'SELECT total_amount,balance_amount,status FROM invoices WHERE id=? FOR UPDATE',
      { replacements: [notes[0].invoice_id], transaction: tx },
    );
    assert(invoices.length, 'Invoice not found');
    const amount = Number(notes[0].amount);
    const balance = Number(
      invoices[0].balance_amount ?? invoices[0].total_amount,
    );
    assert(
      amount > 0 && amount <= Number(invoices[0].total_amount),
      'Credit note amount is invalid',
    );
    const nextBalance = Math.min(
      Number(invoices[0].total_amount),
      balance + amount,
    );
    await db.query(
      'UPDATE invoices SET balance_amount=?,status=CASE WHEN ?>=total_amount THEN "draft" ELSE "part_paid" END WHERE id=?',
      {
        replacements: [nextBalance, nextBalance, notes[0].invoice_id],
        transaction: tx,
      },
    );
    await db.query(
      "UPDATE credit_notes SET status='issued' WHERE id=? AND status='draft'",
      { replacements: [id], transaction: tx },
    );
    ...
    await audit(
      ...
      'credit_note',
      id,
      { invoice_id: notes[0].invoice_id, amount },
      tx,
    );
    await tx.commit();
...
async function processImport(db, id, userId) {
  ...
  try {
    ...
    const payload = jobs[0].payload_json
      ? typeof jobs[0].payload_json === 'string'
        ? JSON.parse(jobs[0].payload_json)
        : jobs[0].payload_json
      : [];
    assert(Array.isArray(payload), 'Import payload must be an array');
    const importTargets = {
      item_master: ['item_code', 'item_name', 'category', 'is_active'],
      vendors: ['vendor_code', 'company_name', 'email', 'phone', 'is_active'],
      customers: [
        'customer_code',
        'company_name',
        'email',
    ...
    assert(
      targetColumns,
      'This entity is not supported by the import pipeline',
    );
    await db.query(
      'UPDATE import_jobs SET status="processing",total_rows=?,processed_rows=0,error_json=NULL WHERE id=?',
      { replacements: [payload.length, id], transaction: tx },
    );
    for (let index = 0; index < payload.length; index += 1) {
      ...
      await db.query(
        `INSERT INTO ${jobs[0].entity_type} (id,${columns.join(',')}) VALUES (?,${columns.map(() => '?').join(',')})`,
        { replacements: [uuid(), ...values], transaction: tx },
      );
      await db.query(
        'INSERT INTO import_rows(id,import_job_id,row_no,payload,status) VALUES(?,?,?,?,?) ON DUPLICATE KEY UPDATE payload=VALUES(payload),status="processed",error_message=NULL',
        {
          replacements: [
            uuid(),
            id,
    ...
    await db.query(
      'UPDATE import_jobs SET status="completed",processed_rows=?,error_json=NULL WHERE id=?',
      { replacements: [payload.length, id], transaction: tx },
    );
    await audit(
      db,
...
async function processExport(db, id, userId) {
  ...
  try {
    const [jobs] = await db.query(
      'SELECT * FROM export_jobs WHERE id=? FOR UPDATE',
      { replacements: [id], transaction: tx },
    );
    ...
    assert(
      exportTargets.includes(entity),
      'This entity is not supported by the export pipeline',
    );
    const [rows] = await db.query(`SELECT * FROM ${entity} LIMIT 10000`, {
      transaction: tx,
    });
    const content = JSON.stringify(rows);
    await db.query(
      'UPDATE export_jobs SET status="completed",result_json=?,result_content=? WHERE id=?',
      {
        replacements: [JSON.stringify({ row_count: rows.length }), content, id],
        transaction: tx,
      },
    );
    await audit(
...
async function selectQuotation(db, lineId, userId) {
  ...
  try {
    const [line] = await db.query(
      'SELECT q.rfq_supplier_id,q.item_id,s.rfq_id FROM rfq_quotation_lines q JOIN rfq_suppliers s ON s.id=q.rfq_supplier_id WHERE q.id=? FOR UPDATE',
      { replacements: [lineId], transaction: tx },
    );
    assert(line.length, 'Quotation line not found');
    const [[rfq]] = await db.query(
      'SELECT status FROM rfqs WHERE id=? FOR UPDATE',
      { replacements: [line[0].rfq_id], transaction: tx },
    );
    assert(
      rfq && ['quoted', 'compared', 'selected'].includes(rfq.status),
      'Compare quotations before selecting a vendor',
    );
    await db.query(
      'UPDATE rfq_quotation_lines q JOIN rfq_suppliers s ON s.id=q.rfq_supplier_id SET q.is_selected=0 WHERE s.rfq_id=? AND q.item_id=?',
      { replacements: [line[0].rfq_id, line[0].item_id], transaction: tx },
    );
    await db.query('UPDATE rfq_quotation_lines SET is_selected=1 WHERE id=?', {
      replacements: [lineId],
      transaction: tx,
    });
    await audit(
    ...
    await tx.commit();
    return { id: lineId, selected: true };
  } catch (e) {
    await tx.rollback();
    throw e;
  }
