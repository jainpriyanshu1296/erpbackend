...
async function createOrder(options) {
  const r = client();
  return r
    ? r.orders.create(options)
    : { id: `dev_order_${Date.now()}`, ...options };
}
async function fetchPayment(paymentId) {
  const r = client();
  return r ? r.payments.fetch(paymentId) : null;
}
function verifyPayment({
  ...
  return (
    actual.length === wanted.length && crypto.timingSafeEqual(wanted, actual)
  );
}
