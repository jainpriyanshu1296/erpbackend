...
const defaults = {
  ...
  introduction:
    'Bring purchasing, inventory, production, quality and sales into one organization workspace.',
  features: [
  ...
  benefits: [
    'Keep operational records together',
    'Give teams role-based access',
    'Follow stock movement across warehouses',
  ],
...
async function read(publishedOnly = false) {
  const [rows] = await masterDb.query(
    'SELECT * FROM public_pages WHERE page_key=?',
    { replacements: ['landing'] },
  );
  const row = rows[0];
  return {
    content: row
      ? decode(publishedOnly ? row.published_content : row.draft_content) ||
        defaults
      : defaults,
...
async function save({ content, revision, publish }, actor) {
  ...
  return masterDb.transaction(async (transaction) => {
    await masterDb.query(
      ...
      { replacements: ['landing', JSON.stringify(defaults)], transaction },
    );
    const [rows] = await masterDb.query(
    ...
    await masterDb.query(
      ...
      {
        replacements: [json, actor, ...(publish ? [json] : []), 'landing'],
        transaction,
      },
    );
