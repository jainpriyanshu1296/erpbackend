...
function signToken(user, org) {
  return jwt.sign(
    { sub: user.id, email: user.email, role: user.role, orgId: org && org.id, orgSlug: org && org.slug },
    secret(),
    { expiresIn: process.env.JWT_EXPIRES_IN || '15m' }
  );
}
function auth(req, res, next) {
  const token = (req.headers.authorization || '').replace(/^Bearer\s+/i, '') || readCookie(req, 'erp_access');
  if (!token) return fail(res, 401, 'UNAUTHENTICATED', 'Authentication required');
  try { req.user = jwt.verify(token, secret()); return next(); } catch { return fail(res, 401, 'UNAUTHENTICATED', 'Invalid or expired token'); }
}
...
module.exports = { auth, signToken, readCookie, setSessionCookies, clearSessionCookies, secret };
