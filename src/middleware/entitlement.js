...
const entitlement = asyncHandler(async (req, res, next) => {
  ...
  const expiry = subscription?.expires_at || subscription?.org_trial_ends_at || req.org.trial_ends_at;
  const expired = expiry && new Date(expiry).getTime() < Date.now();
  if (expired || !subscription || !['active', 'trial', 'paid'].includes(subscription.status)) {
    return fail(res, 402, 'SUBSCRIPTION_REQUIRED', 'An active subscription is required');
  }
