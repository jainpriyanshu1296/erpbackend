...
function logApiError(req, res, error) {
  ...
  const payload = {
    ...
    status_code: status,
    error_code: error?.code || res.locals.errorCode || (status >= 500 ? 'INTERNAL_ERROR' : 'HTTP_ERROR'),
    message: error?.message || res.locals.errorMessage || res.statusMessage || 'Request failed',
  ...
  masterDb.query(
    ...
    console.error(JSON.stringify({ type: 'api_error_log_failure', request_id: req.requestId, message: logError.message }));
  });
}
